...
fn visual_tests() {
    ...
    let folders: Vec<(String, PathBuf, &str, PathBooleanOperation)> =
        glob("__fixtures__/visual-tests/*/")
            .expect("Failed to read glob pattern")
            .flat_map(|entry| {
                let dir = entry.expect("Failed to get directory entry");
                ops.iter().map(move |(op_name, op)| {
                    (
    ...
    for (name, dir, op_name, op) in folders {
        ...
        fs::create_dir_all(dir.join("test-results"))
            .expect("Failed to create test-results directory");
            ...
            match event {
                ...
                Event::Tag("path", svg::node::element::tag::Type::Empty, attributes) => {
                    let data = attributes
                        .get("d")
                        .map(|s| s.to_string())
                        .expect("Path data not found");
                    let fill_rule = attributes
                        .get("fill-rule")
                        .map(|v| v.to_string())
        ...
        for path in &result {
            result_svg.push_str(&format!(
                "<path d=\"{}\" {}/>",
                path_to_path_data(path, 1e-4),
                first_path_attributes
            ));
        }
        ...
        let ground_truth_svg =
            fs::read_to_string(&ground_truth_path).expect("Failed to read ground truth SVG");

        let ours_image = render_svg(&result_svg);
        let ground_truth_image = render_svg(&ground_truth_svg);

        let ours_png_path = dir
            .join("test-results")
            .join(format!("{}-ours.png", op_name));
        ours_image
            .save(&ours_png_path)
            .expect("Failed to save our PNG");

        let ground_truth_png_path = dir.join("test-results").join(format!("{}.png", op_name));
        ground_truth_image
            .save(&ground_truth_png_path)
            .expect("Failed to save ground truth PNG");

        failure |= compare_images(
            &ours_image,
            &ground_truth_image,
            ours_image.width(),
            TOLERANCE,
        ...
        assert_eq!(
            result_path_count, ground_truth_path_count,
            "Number of paths doesn't match for test: {}",
            test_name
        );
    }
    if failure {
...
fn compare_images(img1: &DynamicImage, img2: &DynamicImage, width: u32, tolerance: u8) -> bool {
    assert_eq!(
        img1.dimensions(),
        img2.dimensions(),
    ...
    for (x, y, pixel1) in img1.pixels() {
        ...
        for i in 0..4 {
            let difference = (pixel1[i] as i32 - pixel2[i] as i32).abs() as u8;
            if difference > tolerance {
                println!(
                    "Difference {} larger than tolerance {} at [{}, {}], channel {}.",
                    difference, tolerance, x, y, i
                );
                return true;
            }
