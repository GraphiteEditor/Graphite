...
use crate::aabb::{
    bounding_box_around_point, expand_bounding_box, extend_bounding_box, merge_bounding_boxes, AABB,
};
...
pub fn reverse_path_segment(seg: &PathSegment) -> PathSegment {
    match *seg {
        ...
        PathSegment::Arc(start, rx, ry, phi, fa, fs, end) => {
            PathSegment::Arc(end, rx, ry, phi, fa, !fs, start)
        }
    }
}
...
pub fn arc_segment_to_center(seg: &PathSegment) -> Option<PathArcSegmentCenterParametrization> {
    if let PathSegment::Arc(xy1, rx, ry, phi, fa, fs, xy2) = *seg {
        ...
        let mut xy1_prime = rotation_matrix * (xy1 - xy2) * 0.5;
        ...
        let multiplier = ((rx2 * ry2 - rx2 * y1_prime2 - ry2 * x1_prime2)
        ...
        let mut delta_theta = vector_angle(
            vec1,
            DVec2::new(
                (-xy1_prime.x - cx_prime) / rx,
                (-xy1_prime.y - cy_prime) / ry,
            ),
        );
...
pub fn arc_segment_from_center(params: &PathArcSegmentCenterParametrization) -> PathSegment {
    let rotation_matrix = DMat2::from_angle(params.phi);

    let xy1 = rotation_matrix
        * DVec2::new(
            params.rx * params.theta1.cos(),
            params.ry * params.theta1.sin(),
        )
        + params.center;

    let xy2 = rotation_matrix
        * DVec2::new(
            params.rx * (params.theta1 + params.delta_theta).cos(),
            params.ry * (params.theta1 + params.delta_theta).sin(),
        )
        + params.center;

    let fa = (params.delta_theta.abs() > PI) as bool;
...
pub fn arc_segment_to_cubics(seg: &PathSegment, max_delta_theta: f64) -> Vec<PathSegment> {
    if let PathSegment::Arc(start, rx, ry, phi, _, _, end) = *seg {
        if let Some(center_param) = arc_segment_to_center(seg) {
            ...
            let from_unit = DMat3::from_translation(center_param.center)
                * DMat3::from_angle(deg2rad(phi))
                * DMat3::from_scale(DVec2::new(rx, ry));
            ...
            (0..count)
                .map(|i| {
                    ...
                    let matrix =
                        DMat3::from_angle(center_param.theta1 + i as f64 * theta) * from_unit;
        ...
        PathSegment::Arc(start, rx, ry, phi, _, _, end) => {
            if let Some(center_param) = arc_segment_to_center(seg) {
                ...
                if phi == 0.0 || rx == ry {
                    ...
                    if in_interval(-PI, center_param.theta1, theta2)
                        || in_interval(PI, center_param.theta1, theta2)
                    {
                        bounding_box = extend_bounding_box(
                            Some(bounding_box),
                            DVec2::new(center_param.center.x - rx, center_param.center.y),
                        );
                    }
                    if in_interval(-PI / 2.0, center_param.theta1, theta2)
                        || in_interval(3.0 * PI / 2.0, center_param.theta1, theta2)
                    {
                        bounding_box = extend_bounding_box(
                            Some(bounding_box),
                            DVec2::new(center_param.center.x, center_param.center.y - ry),
                        );
                    }
                    if in_interval(0.0, center_param.theta1, theta2)
                        || in_interval(2.0 * PI, center_param.theta1, theta2)
                    {
                        bounding_box = extend_bounding_box(
                            Some(bounding_box),
                            DVec2::new(center_param.center.x + rx, center_param.center.y),
                    ...
                    if in_interval(PI / 2.0, center_param.theta1, theta2)
                        || in_interval(5.0 * PI / 2.0, center_param.theta1, theta2)
                    {
                        bounding_box = extend_bounding_box(
                            Some(bounding_box),
                            DVec2::new(center_param.center.x, center_param.center.y + ry),
                    ...
                    for cubic_seg in cubics {
                        bounding_box = Some(merge_bounding_boxes(
                            bounding_box,
                            &path_segment_bounding_box(&cubic_seg),
                        ));
                    }
...
pub fn split_segment_at(seg: &PathSegment, t: f64) -> (PathSegment, PathSegment) {
    match *seg {
        ...
        PathSegment::Cubic(p0, p1, p2, p3) => {
            ...
            (
                PathSegment::Cubic(p0, p01, p012, p),
                PathSegment::Cubic(p, p123, p23, p3),
            )
        }
        PathSegment::Quadratic(p0, p1, p2) => {
            ...
            (
                PathSegment::Quadratic(p0, p01, p),
                PathSegment::Quadratic(p, p12, p2),
            )
        }
        PathSegment::Arc(start, rx, ry, phi, fa, fs, end) => {
            if let Some(center_param) = arc_segment_to_center(seg) {
                let mid_theta = center_param.theta1 + center_param.delta_theta * t;
                let mid_point = sample_path_segment_at(seg, t);

                let seg1 = PathSegment::Arc(
                    start, rx, ry, phi, false, // large arc flag for first half
                    fs,    // sweep flag remains the same
                    mid_point,
                );

                let seg2 = PathSegment::Arc(
                    mid_point, rx, ry, phi, false, // large arc flag for second half
                    fs,    // sweep flag remains the same
                    end,
                );
    ...
    //             let seg1 = arc_segment_from_center(&PathArcSegmentCenterParametrization {
    //                 delta_theta: mid_delta_theta,
    //                 ..center_param
    //             });
    //             let seg2 = arc_segment_from_center(&PathArcSegmentCenterParametrization {
