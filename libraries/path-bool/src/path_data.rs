...
pub fn commands_from_path_data(d: &str) -> Vec<PathCommand> {
    ...
    while let Some(cmd) = get_cmd(&mut i, last_cmd) {
        last_cmd = cmd;
        match cmd {
            'M' => commands.push(PathCommand::Absolute(AbsolutePathCommand::M(Vector::new(
                get_float(&mut i),
                get_float(&mut i),
            )))),
            'L' => commands.push(PathCommand::Absolute(AbsolutePathCommand::L(Vector::new(
                get_float(&mut i),
                get_float(&mut i),
            )))),
            ...
            'T' => commands.push(PathCommand::Absolute(AbsolutePathCommand::T(Vector::new(
                get_float(&mut i),
                get_float(&mut i),
            )))),
            ...
            'H' => commands.push(PathCommand::Absolute(AbsolutePathCommand::H(get_float(
                &mut i,
            )))),
            'V' => commands.push(PathCommand::Absolute(AbsolutePathCommand::V(get_float(
                &mut i,
            )))),
            'm' => commands.push(PathCommand::Relative(RelativePathCommand::M(
                get_float(&mut i),
                get_float(&mut i),
            ))),
            'l' => commands.push(PathCommand::Relative(RelativePathCommand::L(
                get_float(&mut i),
                get_float(&mut i),
            ))),
            'h' => commands.push(PathCommand::Relative(RelativePathCommand::H(get_float(
                &mut i,
            )))),
            'v' => commands.push(PathCommand::Relative(RelativePathCommand::V(get_float(
                &mut i,
            )))),
            ...
            't' => commands.push(PathCommand::Relative(RelativePathCommand::T(
                get_float(&mut i),
                get_float(&mut i),
            ))),
    ...
    path_to_commands(path.iter(), eps)
        .map(|cmd| match cmd {
            PathCommand::Absolute(abs_cmd) => match abs_cmd {
                ...
                AbsolutePathCommand::C(p1, p2, p3) => format!(
                    "C{:.12},{:.12} {:.12},{:.12} {:.12},{:.12}",
                ...
                AbsolutePathCommand::A(rx, ry, x_axis_rotation, large_arc_flag, sweep_flag, p) => {
                    format!(
                        "A{:.12} {:.12} {:.12} {} {} {:.12},{:.12}",
                        rx, ry, x_axis_rotation, large_arc_flag as u8, sweep_flag as u8, p.x, p.y
                    )
                }
            ...
            PathCommand::Relative(rel_cmd) => match rel_cmd {
                ...
                RelativePathCommand::C(dx1, dy1, dx2, dy2, dx, dy) => format!(
                    "c{:.12},{:.12} {:.12},{:.12} {:.12},{:.12}",
                ...
                RelativePathCommand::A(
                    ...
                    "a{:.12} {:.12} {:.12} {} {} {:.12},{:.12}",
                    rx, ry, x_axis_rotation, large_arc_flag as u8, sweep_flag as u8, dx, dy
                ),
            },
        })
        .collect::<Vec<String>>()
