...
use crate::path_data::path_to_path_data;
use crate::path_segment::{
    get_end_point, get_start_point, path_segment_bounding_box, reverse_path_segment,
    sample_path_segment_at, split_segment_at, PathSegment,
};
    ...
    for component in components {
        for &edge_key in &component.edges {
            ...
            dot.push_str(&format!(
                ...
                (edge.twin.unwrap().0.as_ffi() & 0xFF) - 1
            ));
        }
    }
    dot.push_str("}\n");
...
fn split_at_intersections(
...
) -> (Vec<MajorGraphEdgeStage2>, Option<AABB>) {
    // println!("Starting split_at_intersections with {} edges", edges.len());

    // Step 1: Add bounding boxes to edges
    let with_bounding_box: Vec<MajorGraphEdgeStage2> = edges
        .iter()
        .map(|(seg, parent)| (*seg, *parent, path_segment_bounding_box(seg)))
        .collect();
    // println!("Added bounding boxes to edges");

    // Step 2: Calculate total bounding box
    let total_bounding_box = with_bounding_box
        .iter()
        .fold(None, |acc, (_, _, bb)| Some(merge_bounding_boxes(acc, bb)));

    let total_bounding_box = match total_bounding_box {
        Some(bb) => bb,
        None => return (Vec::new(), None),
    };
    // println!("Calculated total bounding box: {:?}", total_bounding_box);
    ...
    // Helper function to add split
    fn add_split(splits_per_edge: &mut HashMap<usize, Vec<f64>>, i: usize, t: f64) {
        splits_per_edge.entry(i).or_default().push(t);
        // println!("Added split for edge {} at t={}", i, t);
    }

    // Step 4: Find intersections and record split points
    for i in 0..with_bounding_box.len() {
        let edge = &with_bounding_box[i];
        let candidates = edge_tree.find(&edge.2);
        // println!("Edge {}: Found {} candidates", i, candidates.len());
        for &j in &candidates {
            let candidate: &(_, _) = &edges[j];
            let include_endpoints = edge.1 != candidate.1
                || !(vectors_equal(
                    get_end_point(&candidate.0),
                    get_start_point(&edge.0),
                    EPS.point,
                ) || vectors_equal(
                    get_start_point(&candidate.0),
                    get_end_point(&edge.0),
                    EPS.point,
                ));
            let intersection =
                path_segment_intersection(&edge.0, &candidate.0, include_endpoints, &EPS);
            // println!(
            //     "  Checking intersection between edge {} and {}: {:?}",
            //     i, j, intersection
            // );
    ...
    // println!("Splits per edge: {:?}", splits_per_edge);
    ...
    for (i, (seg, parent, _)) in with_bounding_box.into_iter().enumerate() {
        ...
        if let Some(splits) = splits_per_edge.get(&i) {
            ...
            // println!("  Sorted splits: {:?}", splits);
            let mut tmp_seg = seg;
            let mut prev_t = 0.0;
            for &t in splits.iter() {
                if t > 1.0 - EPS.param {
                    // println!("  Breaking due to t > 1 - EPS.param");
                    break;
                }
                let tt = (t - prev_t) / (1.0 - prev_t);
                // println!("  t={}, prev_t={}, tt={}", t, prev_t, tt);
                prev_t = t;
                if tt < EPS.param {
                    // println!("  Skipping due to tt < EPS.param");
                    continue;
                }
                if tt > 1.0 - EPS.param {
                    // println!("  Skipping due to tt > 1 - EPS.param");
                    continue;
                }
                let (seg1, seg2) = split_segment_at(&tmp_seg, tt);
                new_edges.push((seg1, parent, path_segment_bounding_box(&seg1)));
                // println!("  Added new edge, total now: {}", new_edges.len());
                tmp_seg = seg2;
            }
            new_edges.push((tmp_seg, parent, path_segment_bounding_box(&tmp_seg)));
            // println!("  Added final segment, total now: {}", new_edges.len());
        } else {
            new_edges.push((seg, parent, path_segment_bounding_box(&seg)));
            // println!("  Added unsplit edge, total now: {}", new_edges.len());
        }
    }

    // println!("Final number of edges: {}", new_edges.len());
...
fn find_vertices(edges: &[MajorGraphEdgeStage2], bounding_box: AABB) -> MajorGraph {
    // println!("Starting find_vertices");
    // println!("Number of input edges: {}", edges.len());
    ...
    let mut vertex_pair_id_to_edges: HashMap<
        (MajorVertexKey, MajorVertexKey),
        Vec<(MajorGraphEdgeStage2, MajorEdgeKey, MajorEdgeKey)>,
    > = HashMap::new();
    ...
    for (seg, parent, bounding_box) in edges {
        ...
        // println!("Processing edge: {:?}, parent: {}", seg, parent);

        let start_point = get_start_point(seg);
        let end_point = get_end_point(seg);
        // println!("  Start point: {:?}", start_point);
        // println!("  End point: {:?}", end_point);

        let start_vertex = get_vertex(start_point);
        let end_vertex = get_vertex(end_point);
        // println!("  Start vertex: {:?}", start_vertex);
        // println!("  End vertex: {:?}", end_vertex);
        ...
        if let Some(existing_edges) = vertex_pair_id_to_edges.get(&vertex_pair_id) {
            if let Some(existing_edge) = existing_edges
                .iter()
                .find(|(other_seg, ..)| segments_equal(seg, &other_seg.0, EPS.point))
            {
                println!("parent: {parent:0b}");
    ...
    // println!("Finished find_vertices");
    // println!("Number of vertices: {}", graph.vertices.len());
...
fn compute_minor(major_graph: &MajorGraph) -> MinorGraph {
    // println!("Starting compute_minor");
    // println!("Major graph edges: {}", major_graph.edges.len());
    // println!("Major graph vertices: {}", major_graph.vertices.len());
    ...
    for (major_vertex_key, vertex) in &major_graph.vertices {
        // println!("Processing major vertex: {:?}", major_vertex_key);
        // println!("  Outgoing edges: {:?}", vertex.outgoing_edges);

        // Edges are contracted
        if get_order(vertex) == 2 {
            continue;
        }
        let start_vertex = *to_minor_vertex.entry(major_vertex_key).or_insert_with(|| {
            new_vertices.insert(MinorGraphVertex {
                outgoing_edges: Vec::new(),
            })
        });
        ...
        for &start_edge_key in &vertex.outgoing_edges {
                ...
                && get_order(&major_graph.vertices[edge.incident_vertices[1]]) == 2
            {
                ...
                edge_key = *next_vertex
                    .outgoing_edges
                    .iter()
                    .find(|&&e| Some(e) != edge.twin)
                    .unwrap();
            ...
            let twin_id = (
                edge.twin.unwrap(),
                major_graph.edges[start_edge_key].twin.unwrap(),
            );
    ...
    // Ensure all edges have twins
    // let mut insert_edges = Vec::new();
    // for (edge_key, edge) in new_edges.iter_mut() {
    //     if edge.twin.is_none() {
    //         panic!();
    //         insert_edges.push((edge_key, edge.clone()));
    //     }
    // }
    // for (edge_key, edge) in insert_edges.iter_mut() {
    //     let twin_key = new_edges.insert(MinorGraphEdge {
    //         segments: edge
    //             .segments
    //             .iter()
    //             .rev()
    //             .map(reverse_path_segment)
    //             .collect(),
    //         parent: edge.parent,
    //         incident_vertices: [edge.incident_vertices[1], edge.incident_vertices[0]],
    //         direction_flag: !edge.direction_flag,
    //         twin: Some(*edge_key),
    //     });
    //     edge.twin = Some(twin_key);
    //     new_vertices[edge.incident_vertices[1]]
    //         .outgoing_edges
    //         .push(twin_key);
    // }

    // println!("Created {} minor edges", new_edges.len());
    for (edge_key, edge) in &new_edges {
        // println!(
        //     "  Minor edge {:?}: {:?} -> {:?}",
        //     edge_key, edge.incident_vertices[0], edge.incident_vertices[1]
        // );
    }
    ...
    for (major_vertex_key, vertex) in &major_graph.vertices {
        ...
        loop {
            ...
            assert_eq!(
                major_graph.vertices[edge.incident_vertices[1]]
                    .outgoing_edges
                    .len(),
                2,
    ...
    // println!("Finished compute_minor");
    // println!("Minor graph edges: {}", new_edges.len());
    // println!("Minor graph vertices: {}", new_vertices.len());
    // println!("Minor graph cycles: {}", cycles.len());
...
fn remove_dangling_edges(graph: &mut MinorGraph) {
    ...
    fn walk(parent: u8, graph: &MinorGraph) -> HashSet<MinorVertexKey> {
        ...
        fn visit(
            ...
            // println!(
            //     "inserting {}, with level {}",
            //     (vertex.0.as_ffi() & 0xFF) - 1,
            //     level
            // );
            ...
            for &edge_key in &graph.vertices[vertex].outgoing_edges {
                ...
                if edge.parent & parent != 0 && Some(edge_key) != incoming_edge {
                    min_level = min_level.min(visit(
                        edge.incident_vertices[1],
                        edge.twin,
                        level + 1,
                        graph,
                        vertex_to_level,
            ...
            // println!("min_level: {min_level}, level: {level}");
            if min_level <= level {
                // println!(
                //     "keeping {:?}, {}, {}",
                //     (vertex.0.as_ffi() & 0xFF) - 1,
    ...
    // println!("keep a: {kept_vertices_a:?}");
    // println!("keep b: {kept_vertices_b:?}");

    graph
        .vertices
        .retain(|k, _| kept_vertices_a.contains(&k) || kept_vertices_b.contains(&k));

    for vertex in graph.vertices.values_mut() {
        vertex.outgoing_edges.retain(|&edge_key| {
            let edge = &graph.edges[edge_key];
            (edge.parent & 1 == 1
                && kept_vertices_a.contains(&edge.incident_vertices[0])
                && kept_vertices_a.contains(&edge.incident_vertices[1]))
                || (edge.parent & 2 == 2
                    && kept_vertices_b.contains(&edge.incident_vertices[0])
                    && kept_vertices_b.contains(&edge.incident_vertices[1]))
        });
    }
...
fn get_incidence_angle(edge: &MinorGraphEdge) -> f64 {
    ...
    let (p0, p1) = if edge.direction_flag.forward() {
        (
            sample_path_segment_at(seg, 0.0),
            sample_path_segment_at(seg, EPS.param),
        )
    } else {
        (
            sample_path_segment_at(seg, 1.0),
            sample_path_segment_at(seg, 1.0 - EPS.param),
        )
    };
    ...
    for vertex in graph.vertices.values_mut() {
        if vertex.outgoing_edges.len() > 2 {
            vertex.outgoing_edges.sort_by(|&a, &b| {
                get_incidence_angle(&graph.edges[a])
                    .partial_cmp(&get_incidence_angle(&graph.edges[b]))
                    .unwrap()
            });
        }
    }
        ...
        .flat_map(|&edge_key| {
            ...
            edge.segments.iter().flat_map(move |seg| {
                (0..CNT).map(move |i| {
                    ...
                    let t = if edge.direction_flag.forward() {
                        t0
                    } else {
                        1.0 - t0
                    };
...
fn compute_winding(
    ...
    // println!(
    //     "Computing winding for polygon with {} points:",
    //     polygon.len()
    // );
...
fn compute_dual(minor_graph: &MinorGraph) -> DualGraph {
    // println!("Starting compute_dual function");
    // println!("Minor graph edges: {:?}", minor_graph.edges.len());
    // println!("Minor graph vertices: {:?}", minor_graph.vertices.len());
    ...
    for (start_edge_key, start_edge) in &minor_graph.edges {
        // println!("Processing start edge: {:?}", start_edge_key,);

        if minor_to_dual_edge.contains_key(&start_edge_key) {
            //         println!("Edge {:?} already processed, skipping", start_edge_key);
            continue;
        }

        let face_key = dual_vertices.insert(DualGraphVertex {
            incident_edges: Vec::new(),
        });
        //     println!("Created new face vertex: {:?}", face_key);

        let mut edge_key = start_edge_key;
        let mut edge = start_edge;

        loop {
            // println!("Processing edge: {:?}: {}", edge_key, edge.format_path());
            ...
            //         println!("Twin dual key: {:?}", twin_dual_key);
            ...
            //         println!("Next edge: {:?}", edge_key);

            if edge.incident_vertices[0] == start_edge.incident_vertices[0] {
                // println!("Completed face, breaking loop");
                break;
            }
        }

        new_vertices.push(face_key);
        //     println!("Added face vertex {:?} to new_vertices", face_key);
    }

    // println!("Finished processing edges");
    // println!("New vertices: {:?}", new_vertices);

    for cycle in &minor_graph.cycles {
        let inner_face_key = dual_vertices.insert(DualGraphVertex {
            incident_edges: Vec::new(),
        });
        let outer_face_key = dual_vertices.insert(DualGraphVertex {
            incident_edges: Vec::new(),
        });
        ...
        dual_vertices[inner_face_key]
            .incident_edges
            .push(inner_half_edge_key);
        dual_vertices[outer_face_key]
            .incident_edges
            .push(outer_half_edge_key);
    ...
    for &start_vertex_key in &new_vertices {
        ...
        assert_eq!(
            component_vertices
                .iter()
                .filter(|&&face_key| compute_winding(&dual_vertices[face_key], &dual_edges).0 < 0)
                .count(),
            1,
    ...
    // println!("Dual graph components: {}", components.len());
...
fn get_next_edge(edge_key: MinorEdgeKey, graph: &MinorGraph) -> MinorEdgeKey {
    ...
    let index = vertex
        .outgoing_edges
        .iter()
        .position(|&e| Some(edge_key) == graph.edges[e].twin)
        .unwrap();
    vertex.outgoing_edges[(index + 1) % vertex.outgoing_edges.len()]
}

fn test_inclusion(
    a: &DualGraphComponent,
    b: &DualGraphComponent,
    edges: &SlotMap<DualEdgeKey, DualGraphHalfEdge>,
    vertices: &SlotMap<DualVertexKey, DualGraphVertex>,
) -> Option<DualVertexKey> {
...
fn bounding_box_intersects_horizontal_ray(bounding_box: &AABB, point: Vector) -> bool {
    interval_crosses_point(bounding_box.top, bounding_box.bottom, point[1])
        && bounding_box.right >= point[0]
}
...
pub fn path_segment_horizontal_ray_intersection_count(
    orig_seg: &PathSegment,
    point: Vector,
) -> usize {
    // println!("Checking segment: {:?}", orig_seg);
    // println!("Against point: {:?}", point);

    let total_bounding_box = path_segment_bounding_box(orig_seg);
    // println!("Total bounding box: {:?}", total_bounding_box);

    if !bounding_box_intersects_horizontal_ray(&total_bounding_box, point) {
        // println!("Ray doesn't intersect bounding box. Returning 0.");
        return 0;
    }
    ...
    while !segments.is_empty() {
        ...
        for segment in segments {
            // println!("Checking sub-segment: {:?}", segment.seg);
            // println!(
            //     "Segment endpoints: start={:?}, end={:?}",
            //     get_start_point(&segment.seg),
            //     get_end_point(&segment.seg)
            // );
            if bounding_box_max_extent(&segment.bounding_box) < EPS.linear {
                // println!("Segment is small enough. Checking for intersection.");
                if line_segment_intersects_horizontal_ray(
                    get_start_point(&segment.seg),
                    get_end_point(&segment.seg),
                    point,
                ) {
                    count += 1;
                    // println!("Intersection found. Current count: {}", count);
                }
            } else {
                // println!("Subdividing segment.");
                ...
                if bounding_box_intersects_horizontal_ray(&bounding_box0, point) {
                    ...
                    // println!("First half intersects. Added to next iteration.");
                }
                if bounding_box_intersects_horizontal_ray(&bounding_box1, point) {
                    ...
                    // println!("Second half intersects. Added to next iteration.");
                }
            }
        }
    ...
    // println!("Final count: {}, after {} iterations", count, iterations);
    count
}

fn compute_nesting_tree(
    DualGraph {
        components,
        vertices,
        edges,
...
fn insert_component(
    ...
    let mut i = 0;
    while i < trees.len() {
        if let Some(face_key) =
            test_inclusion(&trees[i].component, &new_tree.component, edges, vertices)
        {
...
fn flag_faces(
    ...
    fn visit_tree(
        ...
        fn visit_face(
            face_key: DualVertexKey,
            a_running_count: i32,
            b_running_count: i32,
            tree: &mut NestingTree,
            visited_faces: &mut HashSet<DualVertexKey>,
            a_fill_rule: FillRule,
            b_fill_rule: FillRule,
            edges: &SlotMap<DualEdgeKey, DualGraphHalfEdge>,
            vertices: &SlotMap<DualVertexKey, DualGraphVertex>,
            ...
            let a_flag = get_flag(a_running_count, a_fill_rule);
            let b_flag = get_flag(b_running_count, b_fill_rule);
            *flags.entry(face_key).or_default() = a_flag | (b_flag << 1);

            for edge_key in vertices[face_key].incident_edges.clone() {
                let edge = &edges[edge_key];
                let twin_key = edge.twin.expect("Edge doesn't have a twin");
                let mut next_a_count = a_running_count;
                if edge.parent & 1 != 0 {
                    next_a_count += if edge.direction_flag.forward() { 1 } else { -1 };
                }
                let mut next_b_count = b_running_count;
                if edge.parent & 2 != 0 {
                    next_b_count += if edge.direction_flag.forward() { 1 } else { -1 };
                }
                visit_face(
                    edges[twin_key].incident_vertex,
                    next_a_count,
                    next_b_count,
                    tree,
                    visited_faces,
            ...
            if let Some(subtrees) = tree.outgoing_edges.get_mut(&face_key) {
        ...
        let outer_face_key = tree
            .component
            .outer_face
            .expect("Component doesn't have an outer face.");
        visit_face(
            outer_face_key,
            a_running_count,
            b_running_count,
    ...
    for tree in nesting_trees.iter_mut() {
        visit_tree(tree, 0, 0, a_fill_rule, b_fill_rule, edges, vertices, flags);
    }
}

// TODO(@Truedoctor): Check if we can just iterate over the flags
fn get_selected_faces<'a>(
    nesting_trees: &[NestingTree],
    predicate: &'a impl Fn(u8) -> bool,
    flags: &'a HashMap<DualVertexKey, u8>,
) -> impl Iterator<Item = DualVertexKey> + 'a {
    // flags
    //     .iter()
    //     .filter_map(|(key, &flag)| predicate(flag).then_some(*key))
    let mut selected_faces = Vec::new();
    for tree in nesting_trees {
        for face_key in tree.component.vertices.iter() {
            if predicate(*flags.get(face_key).unwrap()) {
                selected_faces.push(*face_key);
            }
        }
        for subtree in tree.outgoing_edges.values().flatten() {
            selected_faces.extend(get_selected_faces(
    ...
    selected_faces.into_iter()
}

fn walk_faces<'a>(
    faces: &'a HashSet<DualVertexKey>,
    edges: &SlotMap<DualEdgeKey, DualGraphHalfEdge>,
    vertices: &SlotMap<DualVertexKey, DualGraphVertex>,
) -> impl Iterator<Item = PathSegment> + 'a {
    let is_removed_edge = |edge: &DualGraphHalfEdge| {
        faces.contains(&edge.incident_vertex)
            == faces.contains(&edges[edge.twin.unwrap()].incident_vertex)
    };
...
pub fn path_boolean(
    ...
    let mut unsplit_edges: Vec<MajorGraphEdgeStage1> = a
        .iter()
        .map(segment_to_edge(1))
        .chain(b.iter().map(segment_to_edge(2)))
        .collect();

    for edge in &unsplit_edges {
        // println!(" {:?}", &path_to_path_data(&vec![edge.0], 0.001));
    }
    split_at_self_intersections(&mut unsplit_edges);
    // println!("after self intersection");
    // for edge in &unsplit_edges {
    //     // println!(" {:?}", &edge);
    //     println!(" {:?}", &path_to_path_data(&vec![edge.0], 0.001));
    // }
    ...
    // println!(
    //     "split_edges: {:?}\n\n total_bounding_box {:?}",
    //     split_edges, total_bounding_box
    // );
    ...
    for (edge_key, edge) in &minor_graph.edges {
        assert!(
            minor_graph.vertices.contains_key(edge.incident_vertices[0]),
            "Edge {:?} has invalid start vertex",
            edge_key
        );
        assert!(
            minor_graph.vertices.contains_key(edge.incident_vertices[1]),
        ...
        assert_eq!(
            twin.twin.unwrap(),
            edge_key,
            "Twin relationship should be symmetrical for edge {:?}",
            edge_key
        );
    }

    // println!("Minor graph structure before compute_dual:");
    for (vertex_key, vertex) in &minor_graph.vertices {
        //     println!("Vertex {:?}:", vertex_key);
        //     println!("  Outgoing edges: {:?}", vertex.outgoing_edges);
        for &edge_key in &vertex.outgoing_edges {
            let edge = &minor_graph.edges[edge_key];
            //         println!("    Edge {:?}:", edge_key);
            //         println!("      Parent: {}", edge.parent);
            //         println!("      Direction flag: {:?}", edge.direction_flag);
            //         println!("      Incident vertices: {:?}", edge.incident_vertices);
            //         println!("      Twin: {:?}", edge.twin);
            //         println!("      Segments: {}", edge.format_path());
        }
    }
    ...
    // println!(
    //     "{}",
    //     dual_graph_to_dot(&dual_graph.components, &dual_graph.edges)
    // );

    // println!(
    //     "Dual Graph computed. Components: {}",
    //     dual_graph.components.len()
    // );

    let mut nesting_trees = compute_nesting_tree(&dual_graph);
    // println!(
    //     "Nesting trees computed. Number of trees: {}",
    //     nesting_trees.len()
    // );
    ...
    // println!("Faces flagged. Number of flagged faces:",);
    ...
    match op {
        PathBooleanOperation::Division | PathBooleanOperation::Fracture => {
            let result = dump_faces(&nesting_trees, predicate, edges, vertices, &flags);
            // println!(
            //     "Operation result (Division/Fracture). Number of paths: {}",
            //     result.len()
            // );
            result
        }
        _ => {
            let selected_faces: HashSet<DualVertexKey> =
                get_selected_faces(&nesting_trees, &predicate, &flags).collect();
            // println!("Selected faces: {}", selected_faces.len());
            let result = vec![walk_faces(&selected_faces, edges, vertices).collect()];
            // println!("Operation result. Number of paths: {}", result.len());
            result
        }
    }
}
...
mod tests {
    ...
    fn test_segment_to_edge() {
        ...
        let parent = 1;
        let edge = segment_to_edge(parent)(&segment);
        assert_eq!(edge.0, segment);
        assert_eq!(edge.1, parent);
    }

    #[test]
    fn test_split_at_self_intersections() {
        let mut edges = vec![
            (
                PathSegment::Arc(
                    DVec2::new(39.0, 20.0),
            ...
            (
                PathSegment::Line(DVec2::new(39.0, 20.0), DVec2::new(39.0, 20.0)),
                1,
            ),
        ];

        let original_length = edges.len();
        split_at_self_intersections(&mut edges);

        // We don't expect any splits to occur
        assert_eq!(edges.len(), original_length);

        // Add a cubic curve (even if it doesn't self-intersect)
        edges.push((
            PathSegment::Cubic(
                DVec2::new(0.0, 0.0),
                DVec2::new(10.0, 10.0),
        ...
        // We expect the cubic curve to remain unchanged
        assert_eq!(edges.len(), original_length + 1);
    }
    ...
    fn test_split_at_intersections() {
        let unsplit_edges = vec![
            ...
            (
                PathSegment::Line(DVec2::new(47.0, 28.0), DVec2::new(47.0, 28.0)),
                2,
            ),
        ];
    ...
    fn test_compute_minor() {
        ...
        // Assertions
        assert_eq!(
            minor_graph.edges.len(),
            8,
            "Expected 8 edges in minor graph"
        );
        assert_eq!(
            minor_graph.vertices.len(),
            2,
            "Expected 2 vertices in minor graph"
        );
        assert!(
            minor_graph.cycles.is_empty(),
            "Expected no cycles in minor graph"
        );
        ...
        for (vertex_key, vertex) in &minor_graph.vertices {
            assert_eq!(
                vertex.outgoing_edges.len(),
                4,
                "Vertex {:?} should have 4 outgoing edges",
                vertex_key
            );
        }
        ...
        assert_eq!(
            minor_graph.edges.values().filter(|e| e.parent == 1).count(),
            4,
            "Expected 4 edges with parent 1"
        );
        assert_eq!(
            minor_graph.edges.values().filter(|e| e.parent == 2).count(),
            4,
            "Expected 4 edges with parent 2"
        );
    }

    #[test]
    fn test_remove_dangling_edges() {
        // Set up the initial graph
        let unsplit_edges = unsplit_edges();
        let (split_edges, total_bounding_box) = split_at_intersections(&unsplit_edges);
        let major_graph = find_vertices(&split_edges, total_bounding_box.unwrap());
        ...
        let initial_edge_count = minor_graph.edges.len();
        let initial_vertex_count = minor_graph.vertices.len();

        // Apply remove_dangling_edges
        remove_dangling_edges(&mut minor_graph);

        // Print final state
        //     println!("\nAfter remove_dangling_edges:");
        print_minor_graph_state(&minor_graph);
        ...
        assert_eq!(
            minor_graph.edges.len(),
            initial_edge_count,
            "Number of edges should not change if there are no dangling edges"
        );
        assert_eq!(
            minor_graph.vertices.len(),
            initial_vertex_count,
            "Number of vertices should not change if there are no dangling edges"
        );
        assert!(
            minor_graph.cycles.is_empty(),
            "Expected no cycles after removing dangling edges"
        );

        // Check that each vertex has the same number of outgoing edges as before
        for (vertex_key, vertex) in &minor_graph.vertices {
            assert_eq!(
                vertex.outgoing_edges.len(),
                4,
                "Vertex {:?} should have 8 outgoing edges",
                vertex_key
            );
        }
        ...
        // Check that all edges are referenced by vertices
        let all_outgoing_edges: HashSet<_> = minor_graph
            .vertices
            .values()
            .flat_map(|v| v.outgoing_edges.iter().cloned())
            .collect();
        assert_eq!(
            all_outgoing_edges.len(),
            minor_graph.edges.len(),
            "All edges should be referenced by vertices"
        );

        // Check that the graph structure remains unchanged
        assert_eq!(
            minor_graph.edges.len(),
            8,
            "Expected 16 edges after removing dangling edges"
        );
        assert_eq!(
            minor_graph.vertices.len(),
            2,
            "Expected 2 vertices after removing dangling edges"
        );
    }
    ...
    fn test_sort_outgoing_edges_by_angle() {
        ...
        let initial_edge_order: HashMap<MinorVertexKey, Vec<MinorEdgeKey>> = minor_graph
            .vertices
            .iter()
            .map(|(k, v)| (k, v.outgoing_edges.clone()))
            .collect();
        ...
        assert_eq!(
            minor_graph.edges.len(),
            8,
            "Number of edges should remain unchanged"
        );
        assert_eq!(
            minor_graph.vertices.len(),
            2,
            "Number of vertices should remain unchanged"
        );
        ...
        for (vertex_key, vertex) in &minor_graph.vertices {
            let angles: Vec<f64> = vertex
                .outgoing_edges
                .iter()
                .map(|&edge_key| get_incidence_angle(&minor_graph.edges[edge_key]))
                .collect();
            ...
            for i in 1..angles.len() {
                assert!(
                    angles[i] >= angles[i - 1],
                    "Edges for vertex {:?} are not sorted by angle",
                    vertex_key
                );
            }
            ...
            assert_eq!(
                initial_edges, sorted_edges,
                "Set of edges for vertex {:?} changed after sorting",
                vertex_key
            );
        }
    ...
    fn test_path_segment_horizontal_ray_intersection_count() {
        let orig_seg = PathSegment::Arc(
            DVec2::new(24.0, 10.090978),
            13.909023,
            13.909023,
            0.0,
            false,
    ...
    fn test_bounding_box_intersects_horizontal_ray() {
        ...
        assert!(bounding_box_intersects_horizontal_ray(
            &bbox,
            DVec2::new(0.0, 30.0)
        ));
        assert!(bounding_box_intersects_horizontal_ray(
