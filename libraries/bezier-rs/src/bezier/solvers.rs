...
mod tests {
	...
	fn test_de_casteljau_points() {
		...
		assert_eq!(expected_de_casteljau_points[3][0], bezier.evaluate(ComputeType::Parametric { t: 0.5 }));
	}
	...
	fn test_intersect_line_segment_linear() {
		...
		assert!(compare_points(bezier.evaluate(ComputeType::Parametric { t: intersections1[0] }), DVec2::new(30., 60.)));
		...
		assert!(compare_points(bezier.evaluate(ComputeType::Parametric { t: intersections2[0] }), DVec2::new(96., 96.)));
	}
	...
	fn test_intersect_line_segment_quadratic() {
		...
		let intersections1 = bezier.intersections(&line1, None);
		assert!(intersections1.len() == 1);
		assert!(compare_points(bezier.evaluate(ComputeType::Parametric { t: intersections1[0] }), p1));

		// Intersection in the middle of curve
		let line2 = Bezier::from_linear_coordinates(150., 150., 30., 30.);
		let intersections2 = bezier.intersections(&line2, None);
		assert!(compare_points(bezier.evaluate(ComputeType::Parametric { t: intersections2[0] }), DVec2::new(47.77355, 47.77354)));
	}
	...
	fn test_intersect_line_segment_cubic() {
		...
		assert!(compare_points(bezier.evaluate(ComputeType::Parametric { t: intersections2[0] }), p1));
		assert!(compare_points(bezier.evaluate(ComputeType::Parametric { t: intersections2[1] }), DVec2::new(85.84, 85.84)));
	}
	...
	fn test_intersect_with_self() {
		...
		assert!(compare_vec_of_points(
			intersections.iter().map(|&t| bezier.evaluate(ComputeType::Parametric { t: t[0] })).collect(),
			intersections.iter().map(|&t| bezier.evaluate(ComputeType::Parametric { t: t[1] })).collect(),
			2.
		));
