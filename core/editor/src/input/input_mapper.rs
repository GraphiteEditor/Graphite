...
impl Default for Mapping {
	fn default() -> Self {
		let (up, down, pointer_move) = mapping![
			...
			entry! {action=DocumentMessage::RotateUp, key_up=Mmb, modifiers=[KeyShift]},
			entry! {action=DocumentMessage::TranslateDown, key_down=Mmb},
			entry! {action=DocumentMessage::TranslateUp, key_up=Mmb},
			entry! {action=DocumentMessage::ZoomIn, key_down=KeyPlus},
			entry! {action=DocumentMessage::ZoomIn, key_down=KeyEquals},
			entry! {action=DocumentMessage::ZoomOut, key_down=KeyMinus},
