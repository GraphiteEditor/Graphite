...
use crate::tools::Fsm;
use crate::{
	communication::{message::prelude::*, AsMessage, InputPreprocessor, Message, MessageDiscriminant, MessageHandler},
	tools::{DocumentToolData, ToolActionHandlerData, ToolMessage},
	SvgDocument,
};
...
#[message(Message, ToolMessage, Rectangle)]
...
impl<'a> MessageHandler<RectangleMessage, ToolActionHandlerData<'a>> for Rectangle {
	fn process_action(&mut self, action: RectangleMessage, data: ToolActionHandlerData<'a>, responses: &mut Vec<Message>) {
		self.fsm_state = self.fsm_state.transition(action.into(), data.0, data.1, &mut self.data, data.2, responses);
	}
	actions_fn!();
}
