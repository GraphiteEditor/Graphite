...
impl MovementMessageHandler {
	fn create_document_transform_from_layerdata(&self, document: &DocumentMessageHandler, viewport_size: &ViewportPosition, responses: &mut VecDeque<Message>) {
		let half_viewport = viewport_size.as_dvec2() / 2.;
		let layerdata = document.layerdata(&[]);
...
impl MessageHandler<MovementMessage, (&mut DocumentMessageHandler, &InputPreprocessor)> for MovementMessageHandler {
	fn process_action(&mut self, message: MovementMessage, data: (&mut DocumentMessageHandler, &InputPreprocessor), responses: &mut VecDeque<Message>) {
		...
		match message {
			...
			RotateCanvasBegin { snap } => {
				self.rotating = true;
				self.snapping = snap;
				let layerdata = document.layerdata_mut(&[]);
			...
			MouseMove => {
				if self.translating {
					...
					let transformed_delta = document.document.root.transform.inverse().transform_vector2(delta);

					let layerdata = document.layerdata_mut(&[]);
					layerdata.translation += transformed_delta;
					self.create_document_transform_from_layerdata(document, &ipp.viewport_size, responses);
				}
				if self.rotating {
