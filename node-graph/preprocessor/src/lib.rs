...
impl Preprocessor {
	...
	pub fn new() -> Self {
		...
		for (id, metadata) in core_types::registry::NODE_METADATA.lock().unwrap().iter() {
			...
			// A `destructure_output` node is multi-output: it always needs a substitution so its generated network can export
			// each field of its returned struct of wires through a hidden extractor node
			let destructure = output_fields.as_ref();

			// A multi-output node is otherwise evaluated once per connected output, so when a Memoize implementation is registered
			// for its struct (in either rank form), wrap the struct in one so all the extractors share a single evaluation
			let memoize = *memoize
				|| destructure.is_some_and(|destructure| {
					let struct_type = first_node_io.return_value.nested_type();
					into_node_registry.get(&graphene_core::memo::memoize::IDENTIFIER).is_some_and(|implementations| {
						implementations.keys().any(|node_io| {
							let memoized_type = node_io.return_value.nested_type();
							memoized_type == struct_type || *memoized_type == destructure.mapped_type
						})
					})
				});
			...
			if destructure.is_none_or(|destructure| !destructure.has_primary) {
...
mod destructure_tests {
	...
	/// Test-only struct of wires with a `#[primary]` field, exercising the primary-output layout and the
	/// unmemoized path (no Memoize implementation is registered for this struct).
	...
	#[node_macro::node(category(""), destructure_output)]
	...
	/// Test-only struct of mixed ranks returned by a whole-list node, which maps its input list to one output and reduces
	/// it to another from a single evaluation.
	#[derive(Debug, Clone, dyn_any::DynAny, node_macro::Destructure)]
	...
	#[test]
	fn framed_multi_output_node_lifts_each_field_into_the_twin() {
		let destructure = registry::MULTI_OUTPUT_NODES
			.get(&sum_product::IDENTIFIER)
			.expect("Sum Product should be registered as a multi-output node");
		assert_eq!(destructure.mapped_type, concrete!(SumProductList));
		assert!(destructure.fields.iter().all(|field| field.ty == core_types::item!(f64) && field.mapped_ty == core_types::list!(f64)));

		// The node registers a mapped variant returning the twin alongside its single-item form
		let node_registry = core_types::registry::NODE_REGISTRY.lock().unwrap();
		...
		let node = DocumentNode {
			inputs: vec![NodeInput::value(TaggedValue::F64Array(vec![1., 2., 3.]), false), NodeInput::value(TaggedValue::F64(10.), false)],
			implementation: DocumentNodeImplementation::ProtoNode(sum_product::IDENTIFIER),
			..Default::default()
		};
		let mut network = multi_output_into_add_network(node, [0, 1]);
		Preprocessor::new().preprocess(&mut network, &|_| None).expect("Preprocessing should succeed");
		assert_execution_result(network, TaggedValue::F64Array(vec![21., 32., 43.]));
	}
	...
	fn whole_list_multi_output_node_returns_fields_of_mixed_rank() {
		...
		let node = DocumentNode {
			inputs: vec![NodeInput::value(TaggedValue::F64Array(vec![1., 2., 3.]), false)],
		...
		Preprocessor::new().preprocess(&mut network, &|_| None).expect("Preprocessing should succeed");
		assert_execution_result(network, TaggedValue::F64Array(vec![5., 7., 9.]));
	}
