...
use crate::mesh_gradient::{
	...
	cache::{CachedTexture, EvaluatorKey, MeshGradientOutputCache, texture_key},
	renderer::Renderer,
...
pub struct MeshGradientPipelineArgs<'a> {
	...
	pub output_cache: &'a MeshGradientOutputCache,
}
...
impl AsyncWgpuPipeline for MeshGradientPipeline {
	...
	async fn run<'a>(&'a self, executor: &'a wgpu_executor::WgpuExecutor, args: &'a Self::Args<'_>) -> Self::Out {
		...
		if let Some(cached_texture) = args.output_cache.get_cloned::<CachedTexture>(&args.footprint)
			&& cached_texture.key == texture_key
			&& let Some(texture) = cached_texture.texture.upgrade()
		{
		...
		let rendered_texture = self.renderer.render(executor, texture_size, vertices.as_slice(), indices.as_slice(), &color_data, metadata)?;

		args.output_cache.store(
			&args.footprint,
			CachedTexture {
				key: texture_key,
