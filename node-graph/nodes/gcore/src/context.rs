...
fn read_graphic(ctx: impl Ctx + ExtractVarArgs) -> Item<List<Graphic>> {
	let Ok(var_arg) = ctx.vararg(0) else { return Item::new_from_element(Default::default()) };
	let var_arg = var_arg as &dyn std::any::Any;

	Item::new_from_element(var_arg.downcast_ref().cloned().unwrap_or_default())
}

#[node_macro::node(category("Context"), path(graphene_core::vector))]
fn read_vector(ctx: impl Ctx + ExtractVarArgs) -> Item<List<Vector>> {
	let Ok(var_arg) = ctx.vararg(0) else { return Item::new_from_element(Default::default()) };
	let var_arg = var_arg as &dyn std::any::Any;
...
#[node_macro::node(category("Context"), path(graphene_core::vector))]
fn read_raster(ctx: impl Ctx + ExtractVarArgs) -> Item<List<Raster<CPU>>> {
	let Ok(var_arg) = ctx.vararg(0) else { return Item::new_from_element(Default::default()) };
	let var_arg = var_arg as &dyn std::any::Any;
...
#[node_macro::node(category("Context"), path(graphene_core::vector))]
fn read_color(ctx: impl Ctx + ExtractVarArgs) -> Item<List<Color>> {
	let Ok(var_arg) = ctx.vararg(0) else { return Item::new_from_element(Default::default()) };
	let var_arg = var_arg as &dyn std::any::Any;
...
#[node_macro::node(category("Context"), path(graphene_core::vector))]
fn read_gradient(ctx: impl Ctx + ExtractVarArgs) -> Item<List<GradientStops>> {
	let Ok(var_arg) = ctx.vararg(0) else { return Item::new_from_element(Default::default()) };
	let var_arg = var_arg as &dyn std::any::Any;
...
async fn read_position(
	...
	let loop_level = loop_level.into_element();
