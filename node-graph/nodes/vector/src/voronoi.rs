...
///
/// Returns an empty vector when there are fewer than three points or they are all collinear (no triangle exists).
...
///
/// Returns one closed polygon per site that produces a non-empty cell (degenerate or fully-clipped cells are omitted, so
/// the result may be shorter than `sites`). Returns an empty vector when no triangulation exists (fewer than three points
/// or all collinear).
...
/// Performs a single Lloyd relaxation step: moves every interior site to its Voronoi cell centroid, leaving the pinned
/// convex-hull (perimeter) sites in place.
...
/// per-site flag marking the convex-hull (perimeter) sites. A cell is `None` when the site has no incident triangle (e.g. a
/// coincident duplicate) or its cell vanishes after clipping.
...
/// Walks the Delaunay triangles incident to `site` and collects their circumcenters in order, forming the site's Voronoi
/// cell polygon. The polygon is closed for interior sites and open (a fan ending at the hull) for hull sites. Returns
/// `None` for a site with no incident triangle (e.g. a coincident duplicate point).
...
/// Clips `subject` to the convex polygon `clip` using the Sutherland–Hodgman algorithm. The clip polygon may wind either
/// way; the subject need not be convex. Returns the clipped polygon (empty if it lies entirely outside the clip region).
...
mod tests {
	...
	fn delaunay_degenerate_inputs_produce_no_triangles() {
		...
		let collinear = vec![DVec2::new(0., 0.), DVec2::new(1., 1.), DVec2::new(2., 2.)];
		assert!(delaunay_triangles(&collinear).is_empty());
	}
