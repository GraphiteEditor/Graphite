...
async fn inscribe_circles(
	_: impl Ctx,
	mut source: Item<Vector>,
	#[hard_min(0.)]
	#[default(10.)]
	radius: Item<PixelLength>,
) -> Item<Vector> {
	let transform = source.attribute_cloned_or_default::<DAffine2>(ATTR_TRANSFORM);
	...
	for point_index in 0..point_ids_count {
		...
		let Some(pos) = bezpath_algorithms::inscribe_circles_algorithms::inscribe(first, second, *radius.element()) else {
			continue;
		};
