...
use vector_types::mesh_gradient::mesh_gradient_to_bounds_transform;
...
async fn fill<V>(
	...
	content: Item<V>,
	#[default(Color::BLACK)] paint: impl Node<'n, Context<'static>, Output = List<Graphic>>,
	...
	Item<V>: VectorItemMut + 'n + Send,
{
	...
	let mut content = content;
	let paint_target_bounds = paint_target_bounds(&mut content);
	...
	let mut paint_ctx = OwnedContextImpl::from(ctx.clone()).with_paint_render_params(PaintRenderParams { paint_target_bounds });
	...
	let stamped_gradient_transform = needs_gradient_transform.then(|| {
		...
		initial_gradient_transform_for_bounding_box(paint_target_bounds)
	});

	let stamped_mesh_transform = needs_mesh_transform.then(|| mesh_gradient_to_bounds_transform(paint_target_bounds));
