...
#[node_macro::node(category("Gradient"))]
fn gradient_reverse(_: impl Ctx, (gradient, cyclic): (Gradient, Attr<GradientCyclicAttr>)) -> Gradient {
	gradient.reversed(cyclic.0)
}
...
fn gradient_shift(
	...
	gradient.shift_positions(fraction, cyclic.0);
	gradient
}
...
fn gradient_stretch(
	...
	gradient.stretch_positions(factor, pivot, cyclic.0);
	gradient
}
