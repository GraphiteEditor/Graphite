...
pub fn place_text_on_path(
	...
	let layout = crate::TextContext::with_thread_local(|ctx| ctx.layout_text(text, font, typesetting));
	...
	layout.lines().for_each(|line| {
		...
		line.items().for_each(|item| {
			if let PositionedLayoutItem::GlyphRun(glyph_run) = item {
				...
				let Ok(font_ref) = ReadFontsRef::from_index(run_font.data.as_ref(), run_font.index) else { return };
						...
						if let Some(glyph_outline) = outlines.get(skrifa::GlyphId::from(glyph.id)) {
							match method {
								TextPathMethod::Align => {
									...
									path_builder.draw_glyph_with_transform(&glyph_outline, font_size, &normalized_coords, style_skew, final_transform);
								}
	...
	for vector in result.iter_element_values_mut() {
		vector.text_on_path_metadata = Some(Arc::clone(&metadata));
	}
