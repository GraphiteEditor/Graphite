...
mod test {
	...
	fn gradient_to_colors_round_trips_the_stops_with_their_placement() {
		...
		let colors = gradient_to_colors(&(), gradient.clone());
		...
		assert_eq!(&restored, &gradient, "wrapping the stops back up should restore the uneven placement rather than redistributing them");
	}
	...
	fn gradient_to_colors_leaves_an_untouched_ramp_without_placement_attributes() {
		...
		let restored = colors_to_gradient(&(), gradient_to_colors(&(), gradient.clone()));
		assert_eq!(&restored, &gradient, "a default ramp should round trip without gaining attributes it never had");
	}
}
