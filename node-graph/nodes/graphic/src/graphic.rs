...
use core_types::{ATTR_EDITOR_LAYER_PATH, ATTR_EDITOR_MERGED_LAYERS, ATTR_SORT_KEY, ATTR_TRANSFORM, AnyHash, BlendMode, CacheHash, CloneVarArgs, Color, Context, Ctx, ExtractAll, OwnedContextImpl};
...
/// Pairwise ordering used by the Sort node for element values and sort keys. Types without a natural
...
/// The "sort_key" column's pairwise ordering by item index, comparing keys with their own [`ElementOrder`], or `None`
/// if the attribute is absent or of an unsortable type. Type-erased column storage requires probing each sortable type.
fn sort_key_ordering<T>(list: &List<T>) -> Option<Box<dyn Fn(usize, usize) -> Ordering + '_>> {
	fn typed_column<K: ElementOrder + 'static, T>(list: &List<T>) -> Option<Box<dyn Fn(usize, usize) -> Ordering + '_>> {
		let keys = list.iter_attribute_values::<K>(ATTR_SORT_KEY)?.as_slice();
		Some(Box::new(move |a, b| keys[a].element_order(&keys[b])))
	}

	typed_column::<f64, _>(list)
		.or_else(|| typed_column::<f32, _>(list))
		.or_else(|| typed_column::<u32, _>(list))
		.or_else(|| typed_column::<u64, _>(list))
		.or_else(|| typed_column::<String, _>(list))
		.or_else(|| typed_column::<bool, _>(list))
}

/// Reorders the items from smallest to largest, either by each item's own value or by its `sort_key` attribute (see the 'Sort Key' node) if the list has one, consuming the used keys.
/// Tied items, and items of types with no natural ordering, keep their original relative order.
#[node_macro::node(category("General"))]
fn sort<T: ElementOrder + Clone + Send + Sync + 'static>(
	...
	list: List<T>,
	/// Reverses the order so the largest value or key comes first.
	reverse: Item<bool>,
) -> List<T> {
	let reverse = reverse.into_element();

	// Order by the "sort_key" attribute column when a sortable one is present, otherwise by the element values themselves
	let key_ordering = sort_key_ordering(&list);
	let elements: Vec<&T> = list.iter_element_values().collect();

	let mut order: Vec<usize> = (0..list.len()).collect();
	order.sort_by(|&a, &b| {
		let ordering = match &key_ordering {
			Some(keys) => keys(a, b),
			None => elements[a].element_order(elements[b]),
		};
	...
	// Consume the used key column so stale keys can't reorder a later Sort
	if key_ordering.is_some() {
		result.remove_attribute(ATTR_SORT_KEY);
	}

	result
}

/// Attaches a per-item sort key attribute (`sort_key`) which the 'Sort' node uses to reorder the items according to the sort order of the keys.
#[node_macro::node(category("General"))]
async fn sort_key<T: AnyHash + Clone + Send + Sync + CacheHash>(
	ctx: impl ExtractAll + CloneVarArgs + Ctx,
	/// The `List` of data to give sort keys to.
	#[implementations(
		List<String>,
		List<bool>,
		List<f32>,
	...
	content: List<T>,
	/// The node that produces the sort key for each item. Called once per item with the item's index in context.
	#[implementations(Context -> Item<AttributeValueDyn>)]
	key: impl Node<'n, Context<'static>, Output = Item<AttributeValueDyn>>,
) -> List<T> {
	write_attribute(ctx, content, Item::new_from_element(ATTR_SORT_KEY.to_string()), key).await
}
...
mod test {
	...
	fn sorts_elements_by_their_natural_order() {
		...
		let sorted = sort((), list, Item::new_from_element(false));
	...
	fn sorts_elements_in_reverse() {
		...
		let sorted = sort((), list, Item::new_from_element(true));
		assert_eq!(elements(&sorted), [3., 2., 1.]);
	}

	#[test]
	fn sort_key_attribute_overrides_element_order_and_is_consumed() {
		let mut list = list_of(["apple".to_string(), "banana".to_string(), "cherry".to_string()]);
		for (index, key) in [2., 0., 1.].into_iter().enumerate() {
			list.set_attribute(ATTR_SORT_KEY, index, key);
		}

		let sorted = sort((), list, Item::new_from_element(false));
		assert_eq!(elements(&sorted), ["banana", "cherry", "apple"]);

		assert!(sorted.iter_attribute_values::<f64>(ATTR_SORT_KEY).is_none(), "the used sort_key column should be consumed by sorting");
	}

	#[test]
	fn integer_sort_keys_coerce_to_numbers() {
		let mut list = list_of(["a".to_string(), "b".to_string(), "c".to_string()]);
		for (index, key) in [2_u32, 0, 1].into_iter().enumerate() {
			list.set_attribute(ATTR_SORT_KEY, index, key);
		}
	...
	#[test]
	fn text_sort_keys_order_items_alphabetically() {
		let mut list = list_of([1., 2., 3.]);
		for (index, key) in ["c", "a", "b"].into_iter().enumerate() {
			list.set_attribute(ATTR_SORT_KEY, index, key.to_string());
		}
