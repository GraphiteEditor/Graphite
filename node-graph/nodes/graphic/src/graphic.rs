	...
	fn flatten_list(output_graphic_list: &mut List<Graphic>, current_graphic_list: List<Graphic>, fully_flatten: bool, recursion_depth: usize) {
		for index in 0..current_graphic_list.len() {
			...
			match current_element {
				...
				// Push any leaf elements we encounter: either `Graphic::GraphicList(...)` values beyond the recursion depth, or non-`Graphic::GraphicList` variants (e.g. `Graphic::VectorList`, `Graphic::Raster*`, `Graphic::ColorList`, `Graphic::Gradient`, `Graphic::Text`)
				_ => {
