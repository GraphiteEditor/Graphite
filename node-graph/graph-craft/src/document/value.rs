...
impl<'a> TaggedValue {
	...
	pub fn to_any(self) -> Any<'a> {
		match self {
			...
			TaggedValue::PointIds(x) => Box::new(x),
	...
	pub fn ty(&self) -> Type {
		match self {
			...
			TaggedValue::Subpaths(_) => concrete!(Vec<bezier_rs::Subpath<graphene_core::vector::PointId>>),
			TaggedValue::RcSubpath(_) => concrete!(Arc<bezier_rs::Subpath<graphene_core::vector::PointId>>),
			...
			TaggedValue::PointIds(_) => concrete!(Vec<graphene_core::vector::PointId>),
	...
	pub fn try_from_any(input: Box<dyn DynAny<'a> + 'a>) -> Result<Self, String> {
		...
		match DynAny::type_id(input.as_ref()) {
			...
			x if x == TypeId::of::<Vec<bezier_rs::Subpath<graphene_core::vector::PointId>>>() => Ok(TaggedValue::Subpaths(*downcast(input).unwrap())),
			x if x == TypeId::of::<Arc<bezier_rs::Subpath<graphene_core::vector::PointId>>>() => Ok(TaggedValue::RcSubpath(*downcast(input).unwrap())),
			...
			x if x == TypeId::of::<Vec<graphene_core::vector::PointId>>() => Ok(TaggedValue::PointIds(*downcast(input).unwrap())),
