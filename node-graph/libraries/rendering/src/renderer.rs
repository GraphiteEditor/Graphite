		...
		if let Some(element_id) = element_id {
			match self {
				...
				Graphic::RasterCPU(table) => {
					...
					// TODO: Find a way to handle more than the first row
					if let Some(row) = table.iter().next() {
						metadata.local_transforms.insert(element_id, row.attribute_cloned_or_default("transform"));
					}
				}
...
impl Render for Table<Artboard> {
	...
	fn collect_metadata(&self, metadata: &mut RenderMetadata, footprint: Footprint, _element_id: Option<NodeId>) {
		for row in self.iter() {
			let source_node_id: Option<NodeId> = row.attribute_cloned_or_default("source_node_id");
			row.element().collect_metadata(metadata, footprint, source_node_id);
		}
	}
...
impl Render for Table<Vector> {
	fn render_svg(&self, render: &mut SvgRender, render_params: &RenderParams) {
		for row in self.iter() {
			let multiplied_transform: DAffine2 = row.attribute_cloned_or_default("transform");
			let alpha_blending: AlphaBlending = row.attribute_cloned_or_default("alpha_blending");
	...
	fn add_upstream_click_targets(&self, click_targets: &mut Vec<ClickTarget>) {
		for row in self.iter() {
			...
			click_targets.extend(row.element().stroke_bezier_paths().map(fill).map(|subpath| {
				...
				click_target.apply_transform(row.attribute_cloned_or_default("transform"));
				click_target
			}));
...
impl Render for Table<Raster<CPU>> {
	...
	fn collect_metadata(&self, metadata: &mut RenderMetadata, footprint: Footprint, element_id: Option<NodeId>) {
		...
		if let Some(raster) = self.iter().next() {
			metadata.local_transforms.insert(element_id, raster.attribute_cloned_or_default("transform"));
		}
	}
