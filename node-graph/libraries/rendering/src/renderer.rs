...
use core_types::list::{ATTR_APPEARANCE, ATTR_MESH_GRADIENT_SVG_METHOD, ATTR_TEXTURE};
...
use vector_types::mesh_gradient::{MeshGradient, MeshGradientSvgMethod};
use vello::*;
...
impl Render for List<MeshGradient> {
	...
	// TODO: Implement metadata collection once rasterization can read attributes applied downstream of the Mesh Gradient Value node.
	// Metadata is intentionally not collected to avoid UX confusion because transforms applied downstream of the value node do not affect rasterization.
	fn collect_metadata(&self, _metadata: &mut RenderMetadata, _footprint: Footprint, _element_id: Option<NodeId>, _inherited_appearance: Option<&Appearance>) {}
...
fn render_mesh_gradient_item_svg(item: ItemRef<'_, MeshGradient>, render: &mut SvgRender, render_params: &RenderParams) {
	...
	let svg_method = item.attribute_cloned_or_default::<MeshGradientSvgMethod>(ATTR_MESH_GRADIENT_SVG_METHOD);
	match svg_method {
		MeshGradientSvgMethod::Raster => {
			...
			let placeholder = render
				.deferred_textures
				.iter()
				.find(|(deferred_texture, _)| *deferred_texture == texture)
				.map(|(_, placeholder)| placeholder.clone());

			let placeholder = placeholder.unwrap_or_else(|| {
				let new_placeholder = format!("texture-placeholder-{}", generate_uuid());
				render.deferred_textures.push((texture, new_placeholder.clone()));
				new_placeholder
			});
			...
			render.leaf_tag("image", |attributes| {
				...
				let matrix = format_transform_matrix(texture_transform);
				if !matrix.is_empty() {
					attributes.push(ATTR_TRANSFORM, matrix);
				}
				if opacity < 1. {
				...
				// The value will be replaced to the actual rasterized texture later
				attributes.push("href", placeholder);
			});
		}
		MeshGradientSvgMethod::DisplacementMap => {
			// SVG mesh gradient rendering with `feDisplacementMap` has two stages:
			//
...
fn render_mesh_gradient_item_to_vello(item: ItemRef<'_, MeshGradient>, scene: &mut Scene, parent_transform: DAffine2, context: &mut RenderContext, render_params: &RenderParams) {
	...
	let blend_mode: BlendMode = item.attribute_cloned_or_default(ATTR_BLEND_MODE);
	let opacity: f64 = item.attribute_cloned_or(ATTR_OPACITY, 1.);
	let opacity_fill: f64 = item.attribute_cloned_or(ATTR_OPACITY_FILL, 1.);
	let clip: bool = item.attribute_cloned_or_default(ATTR_CLIPPING_MASK);
	let raster_item = Item::from(Raster::<GPU>::new_gpu(texture))
		.with_attribute(ATTR_BLEND_MODE, blend_mode)
		.with_attribute(ATTR_OPACITY, opacity)
		.with_attribute(ATTR_OPACITY_FILL, opacity_fill)
		.with_attribute(ATTR_CLIPPING_MASK, clip);
	render_raster_gpu_item_to_vello(ItemRef::Item(&raster_item), scene, parent_transform * texture_transform, context, render_params);
}
