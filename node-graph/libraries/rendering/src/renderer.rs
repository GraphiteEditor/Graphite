...
use core_types::{
	...
	ATTR_FONT_SIZE, ATTR_GRADIENT_TYPE, ATTR_GRADIENT_UNITS, ATTR_LETTER_SPACING, ATTR_LETTER_TILT, ATTR_LINE_HEIGHT, ATTR_LOCATION, ATTR_MAX_HEIGHT, ATTR_MAX_WIDTH, ATTR_OPACITY, ATTR_OPACITY_FILL,
	...
	fn render_svg(&self, render: &mut SvgRender, render_params: &RenderParams) {
		...
		for index in 0..self.len() {
			...
			if gradient_units == GradientUnits::ObjectBoundingBox {
				// Standalone gradient path has no shape bbox, so objectBoundingBox falls back to userSpaceOnUse
