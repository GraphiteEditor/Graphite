...
// Graphic
impl From<Table<Self>> for Graphic {
	fn from(graphic: Table<Self>) -> Self {
...
impl From<Vector> for Graphic {
	fn from(vector: Vector) -> Self {
		Self::Vector(Table::new_from_element(vector))
	}
}
...
impl From<Color> for Graphic {
	fn from(color: Color) -> Self {
		Self::Color(Table::new_from_element(color))
	}
}
...
impl From<Option<Color>> for Graphic {
	fn from(color: Option<Color>) -> Self {
		if let Some(color) = color { Self::Color(Table::new_from_element(color)) } else { Self::default() }
	}
}
// Note: Table conversions handled by blanket impl in gcore
...
impl From<GradientStops> for Graphic {
	fn from(gradient: GradientStops) -> Self {
		Self::Gradient(Table::new_from_element(gradient))
	}
}
...
impl Graphic {
	pub fn as_graphic(&self) -> Option<&Table<Self>> {
		match self {
	...
	pub fn as_graphic_mut(&mut self) -> Option<&mut Table<Self>> {
		match self {
			Self::Graphic(graphic) => Some(graphic),
			_ => None,
		}
	...
	pub fn as_raster(&self) -> Option<&Table<Raster<CPU>>> {
		match self {
			Self::RasterCPU(raster) => Some(raster),
			_ => None,
		}
	}
	...
	pub fn had_clip_enabled(&self) -> bool {
		match self {
			Self::Vector(vector) => vector.iter().all(|row| row.alpha_blending.clip),
			Self::Graphic(graphic) => graphic.iter().all(|row| row.alpha_blending.clip),
			Self::RasterCPU(raster) => raster.iter().all(|row| row.alpha_blending.clip),
			Self::RasterGPU(raster) => raster.iter().all(|row| row.alpha_blending.clip),
			Self::Color(color) => color.iter().all(|row| row.alpha_blending.clip),
			Self::Gradient(gradient) => gradient.iter().all(|row| row.alpha_blending.clip),
		}
	}

	pub fn can_reduce_to_clip_path(&self) -> bool {
		match self {
			Self::Vector(vector) => vector.iter().all(|row| {
...
impl BoundingBox for Graphic {
	fn bounding_box(&self, transform: DAffine2, include_stroke: bool) -> RenderBoundingBox {
		match self {
			Self::Vector(vector) => vector.bounding_box(transform, include_stroke),
			Self::RasterCPU(raster) => raster.bounding_box(transform, include_stroke),
			Self::RasterGPU(raster) => raster.bounding_box(transform, include_stroke),
			Self::Graphic(graphic) => graphic.bounding_box(transform, include_stroke),
			Self::Color(color) => color.bounding_box(transform, include_stroke),
			Self::Gradient(gradient) => gradient.bounding_box(transform, include_stroke),
		}
	}
}
