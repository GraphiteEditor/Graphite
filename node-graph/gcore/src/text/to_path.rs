...
#[must_use]
pub fn rich_text_to_path(text: &RichText, line_length: f32, path: &VectorData, font_cache: &FontCache) -> Vec<Subpath<ManipulatorGroupId>> {
...
pub fn buffer_to_path(buffer: &cosmic_text::Buffer, font_system: &mut cosmic_text::FontSystem, spans: &[TextSpan], path: &VectorData) -> Vec<Subpath<ManipulatorGroupId>> {
	...
	let path = path.subpaths.iter().filter(|subpath| !subpath.is_empty()).next();
				...
				for (i, c) in cluster.grapheme_indices(true) {
					...
					if (start.line != run.line_i || c_end > start.index) && (end.line != run.line_i || c_start < end.index) {
						range_opt = match range_opt.take() {
							Some((min, max)) => Some((min.min(c_x + offset_spacing.x as f32) as f32, max.max(c_x + c_w + offset_spacing.x as f32 + spacing as f32) as f32)),
...
pub fn create_buffer(buffer: &mut cosmic_text::Buffer, font_system: &mut cosmic_text::FontSystem, text: &RichText, font_cache: &FontCache, line_length: f32) {
	// Set a size for the text buffer, in pixels
	buffer.set_size(font_system, line_length, f32::MAX);
...
#[must_use]
pub fn create_cosmic_editor(text: &RichText, font_cache: &FontCache, line_length: f32) -> Option<cosmic_text::Editor> {
