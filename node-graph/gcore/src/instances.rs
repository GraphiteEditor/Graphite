...
impl TransformMut for GraphicElement {
	fn transform_mut(&mut self) -> &mut DAffine2 {
		match self {
			GraphicElement::GraphicGroup(group) => group.transform.first_mut().unwrap_or_else(|| panic!("ONE INSTANCE EXPECTED")),
			GraphicElement::VectorData(vector_data) => vector_data.transform.first_mut().unwrap_or_else(|| panic!("ONE INSTANCE EXPECTED")),
