...
pub(crate) fn to_symmetrical_basis_pair(bezier: PathSeg) -> SymmetricalBasisPair {
	...
	for k in 0..q {
		let mut tjk = nck;
		for j in k..q {
			sb.x[j][0] += tjk * pathseg_points_vec(bezier)[k].x;
			sb.x[j][1] += tjk * pathseg_points_vec(bezier)[n - k].x;
			sb.y[j][0] += tjk * pathseg_points_vec(bezier)[k].y;
			sb.y[j][1] += tjk * pathseg_points_vec(bezier)[n - k].y;
		...
		tjk = -nck;
		for j in (k + 1)..q {
			sb.x[j][0] += tjk * pathseg_points_vec(bezier)[n - k].x;
			sb.x[j][1] += tjk * pathseg_points_vec(bezier)[k].x;
			sb.y[j][0] += tjk * pathseg_points_vec(bezier)[n - k].y;
			sb.y[j][1] += tjk * pathseg_points_vec(bezier)[k].y;
	...
	if even {
		...
		for k in 0..q {
			sb.x[q][0] += tjk * (pathseg_points_vec(bezier)[k].x + pathseg_points_vec(bezier)[n - k].x);
			sb.y[q][0] += tjk * (pathseg_points_vec(bezier)[k].y + pathseg_points_vec(bezier)[n - k].y);
			tjk = binomial_increment_k(tjk, n, k);
			tjk = -tjk;
		}
		sb.x[q][0] += tjk * pathseg_points_vec(bezier)[q].x;
		sb.x[q][1] = sb.x[q][0];
		sb.y[q][0] += tjk * pathseg_points_vec(bezier)[q].y;
		sb.y[q][1] = sb.y[q][0];
	}
	sb.x[0][0] = pathseg_points_vec(bezier)[0].x;
	sb.x[0][1] = pathseg_points_vec(bezier)[n].x;
	sb.y[0][0] = pathseg_points_vec(bezier)[0].y;
	sb.y[0][1] = pathseg_points_vec(bezier)[n].y;
