<script lang="ts">
	import DialogModal from "$lib/components/floating-menus/DialogModal.svelte";
	import LayoutCol from "$lib/components/layout/LayoutCol.svelte";
	import LayoutRow from "$lib/components/layout/LayoutRow.svelte";
	import Panel from "$lib/components/window/workspace/Panel.svelte";
	...
	$: documentTabLabels = portfolio.state.documents.map(
		(doc: FrontendDocumentDetails) => {
	...
	function resizePanel(e: PointerEvent) {
		...
		const nextSibling = (gutter?.nextElementSibling || undefined) as
			| HTMLDivElement
			| undefined;
		const prevSibling = (gutter?.previousElementSibling || undefined) as
			| HTMLDivElement
			| undefined;
		const parentElement = (gutter?.parentElement || undefined) as
			| HTMLDivElement
			| undefined;

		const nextSiblingName = (nextSibling?.getAttribute(
			"data-subdivision-name"
		) || undefined) as keyof typeof PANEL_SIZES;
		const prevSiblingName = (prevSibling?.getAttribute(
			"data-subdivision-name"
		) || undefined) as keyof typeof PANEL_SIZES;
		...
		const isHorizontal =
			gutter.getAttribute("data-gutter-horizontal") !== null;

		// Get the current size in px of the panels being resized and the gutter
		const gutterSize = isHorizontal
			? gutter.getBoundingClientRect().width
			: gutter.getBoundingClientRect().height;
		const nextSiblingSize = isHorizontal
			? nextSibling.getBoundingClientRect().width
			: nextSibling.getBoundingClientRect().height;
		const prevSiblingSize = isHorizontal
			? prevSibling.getBoundingClientRect().width
			: prevSibling.getBoundingClientRect().height;
		const parentElementSize = isHorizontal
			? parentElement.getBoundingClientRect().width
			: parentElement.getBoundingClientRect().height;

		// Measure the resizing panels as a percentage of all sibling panels
		const totalResizingSpaceOccupied =
			gutterSize + nextSiblingSize + prevSiblingSize;
		const proportionBeingResized =
			totalResizingSpaceOccupied / parentElementSize;
		...
		const updatePosition = (e: PointerEvent): void => {
			...
			mouseDelta =
				Math.max(nextSiblingSize + mouseDelta, MIN_PANEL_SIZE) -
				nextSiblingSize;
			mouseDelta =
				prevSiblingSize -
				Math.max(prevSiblingSize - mouseDelta, MIN_PANEL_SIZE);

			panelSizes[nextSiblingName] =
				((nextSiblingSize + mouseDelta) / totalResizingSpaceOccupied) *
				proportionBeingResized *
				100;
			panelSizes[prevSiblingName] =
				((prevSiblingSize - mouseDelta) / totalResizingSpaceOccupied) *
				proportionBeingResized *
				100;
