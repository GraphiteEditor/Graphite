...
<script lang="ts">
  ...
  function positionAndStyleFloatingMenu() {
    ...
    if (direction === "Top" || direction === "Bottom") {
      ...
      if (
        ...
        if (workspaceBounds.left + floatingMenuContainerBounds.left === 12)
      ...
      if (
        ...
        if (workspaceBounds.right - floatingMenuContainerBounds.right === 12)
    ...
    if (direction === "Left" || direction === "Right") {
      ...
      if (
        ...
        if (workspaceBounds.top + floatingMenuContainerBounds.top === 12)
      ...
      if (
        ...
        if (workspaceBounds.bottom - floatingMenuContainerBounds.bottom === 12)
  ...
  function hoverTransfer(
    ...
    const ownDescendantMenuSpawners = Array.from(
      self?.parentElement?.querySelectorAll("[data-floating-menu-spawner]") ||
        []
    );
    ...
    while (currentAncestor) {
      ...
      const listOfDescendantSpawners = Array.from(
        currentAncestor?.querySelectorAll("[data-floating-menu-spawner]") || []
      );
      const filteredListOfDescendantSpawners = listOfDescendantSpawners.filter(
  ...
  function isPointerEventOutsideFloatingMenu(
    ...
    return !allContainedFloatingMenus.find(
      (element) =>
        !isPointerEventOutsideMenuElement(e, element, extraDistanceAllowed)
    );
  }
