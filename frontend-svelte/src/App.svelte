...
<script lang="ts">
	...
	onMount(() => {
		...
		Object.assign(managerDestructors, {
			createClipboardManager: createClipboardManager(editor),
			createDragManager: createDragManager(),
			createHyperlinkManager: createHyperlinkManager(editor),
			// TODO: Svelte: maybe remove `.parentElement` now that we're using `body` not `#app`
			createInputManager: createInputManager(editor, self.parentElement, dialog, portfolio, fullscreen),
			createLocalizationManager: createLocalizationManager(editor),
			createPanicManager: createPanicManager(editor, dialog),
			createPersistenceManager: createPersistenceManager(editor, portfolio),
		});

		// Initialize certain setup tasks required by the editor backend to be ready for the user now that the frontend is ready
		const platform = operatingSystem();
		editor.instance.initAfterFrontendReady(platform);
	});
