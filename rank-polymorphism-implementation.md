...
2. **Rank-aware resolution and promote adapters (compiler).** LANDED. The typing context retries failed resolutions with per-input promotion adapters (`ItemToListNode` `Item` -> singleton `List`, plus the `Bundle`/`Unbundle` pair), picking the variant with the fewest promotions and leaving ties ambiguous. Promotions are recorded per node and applied by the executor at construction time as argument wrappers, so the graph structure itself stays adapter-free. Stored values materialize as ranked `Item`/`List` wires directly from `TaggedValue`, which retired the bare-wire promotions (`WrapItemNode`, `WrapListNode`) and the input adapters' bare wrap rows. The preprocessor's generated wrapper networks route ranked fields through `InputAdapterNode` (passthrough or sanctioned element conversion) instead of `IntoNode`/`ConvertNode`.
