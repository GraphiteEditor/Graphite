...
8. **Deferred beyond this branch (explicitly de-scoped).** Copy to Points and Repeat on Points stay, fully ranked and working; their deletion in favor of Transform broadcast waits until after the merge, and Repeat on Points may additionally wait on the eager-to-lazy refactor since it evaluates its content per element with a unique index/position context that Transform's zip does not yet supply. The Brush trace `Item<BrushTrace>` newtype and the assign/spread family are likewise post-merge work; Assign Colors keeps its whole-collection form (verified working in-app).
9. **Later horizons.** Data-tree spines (rank >= 2 as data), demand-driven broadcast-as-re-evaluation in the adapters, frame-total exposure (which would let Assign Colors go element-wise), and possibly bare-`T` kernel sugar once the semantics are settled.

## Remaining before merge

- **Switch under rank-0 wires.** Its lazy branch rows are `Context -> bare/List` only, and lazy connectors receive no promotion adapters, so an `Item`-typed wire into a branch fails to resolve. Needs either mechanical `Context -> Item<X>` rows now or the framed-switch redesign (lazy branch primary plus ranked `Item<bool>` condition); decision pending.
- **Layer-conversion node design.** Decide the intended split between auto-conversion, Wrap Graphic, and To Graphic (which to keep, which to subsume); decision pending.
- **Fill's paint connector.** Whether the paint connector becomes a ranked per-element zip (audit resolution 5) or stays a whole-`List` broadcast; needs a design pass over the picker/value write paths before touching.
