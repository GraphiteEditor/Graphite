...
mod test {
	...
	impl Instrumented {
		...
		/// Pulls a concrete output type out of what the monitor introspects as.
		/// Our monitor stores only the context snapshot, so `introspect` recreates the value itself rather than an `IORecord`.
		...
		pub fn grab_all_input<'a, Input: NodeParameter + 'a, Output: Send + Sync + Clone + 'static>(&'a self, runtime: &'a NodeRuntime) -> impl Iterator<Item = Output> + 'a {
		...
		/// Grabs a ranked input's recorded value as its bare element `Element`.
		/// `introspect` serves a rank-0 input as its element, so no cell unwrapping is needed here.
