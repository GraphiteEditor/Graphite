...
mod test {
	...
	fn copy_paste_single_layer() {
		...
		editor.handle_message(PortfolioMessage::PasteIntoFolder {
			...
			parent: LayerNodeIdentifier::new(
				document_before_copy.network.previewing.expect("Root node should exist in copy_paste_single_layer").id,
	...
	fn copy_paste_single_layer_from_middle() {
		...
		editor.handle_message(PortfolioMessage::PasteIntoFolder {
			clipboard: Clipboard::Internal,
			parent: LayerNodeIdentifier::new(
				document_before_copy.network.previewing.expect("Root node should exist in copy_paste_single_layer_from_middle").id,
				&document_before_copy.network,
			),
			insert_index: -1,
		});
	...
	fn copy_paste_deleted_layers() {
		...
		editor.handle_message(PortfolioMessage::PasteIntoFolder {
			clipboard: Clipboard::Internal,
			parent: LayerNodeIdentifier::new(
				document_before_copy.network.previewing.expect("Root node should exist in copy_paste_deleted_layers").id,
				&document_before_copy.network,
			),
			insert_index: -1,
		});
