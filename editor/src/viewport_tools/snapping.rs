...
impl SnapHandler {
	...
	fn update_overlays(
		...
		let mut index = 0;
		for (x_target, distance) in positions_distances.0.filter(|(_pos, dist)| dist.abs() < 100.) {
			add_overlay_line(
				responses,
				...
				if closest_distance.x == distance { 1. } else { 0.5 - distance.abs() / 200. },
				index,
				overlay_paths,
			);
			index += 1;
		}
		for (y_target, distance) in positions_distances.1.filter(|(_pos, dist)| dist.abs() < 100.) {
			add_overlay_line(
				responses,
				DAffine2::from_scale_angle_translation(DVec2::new(viewport_bounds.x, 1.), 0., DVec2::new(0., (y_target + 0.5).round() - 0.5)).to_cols_array(),
				if closest_distance.y == distance { 1. } else { 0.5 - distance.abs() / 200. },
				index,
				overlay_paths,
			);
			index += 1;
