...
pub fn input_mappings(zoom_with_scroll: bool) -> Mapping {
	...
	let mappings = mapping![
		...
		entry!(KeyDown(PageUp); action_dispatch=TextToolMessage::MoveCursorTop { select: false }),
		entry!(KeyDown(PageDown); action_dispatch=TextToolMessage::MoveCursorBottom { select: false }),
		...
		entry!(KeyDown(PageUp); modifiers=[Shift], action_dispatch=TextToolMessage::MoveCursorTop { select: true }),
		entry!(KeyDown(PageDown); modifiers=[Shift], action_dispatch=TextToolMessage::MoveCursorBottom { select: true }),
		...
		entry!(KeyDown(ArrowUp); modifiers=[Shift, Control], action_dispatch=TextToolMessage::MoveCursorTop { select: true }),
		entry!(KeyDown(ArrowDown); modifiers=[Shift, Command], action_dispatch=TextToolMessage::MoveCursorBottom { select: true }),
		entry!(KeyDown(ArrowDown); modifiers=[Shift, Control], action_dispatch=TextToolMessage::MoveCursorBottom { select: true }),
		...
		entry!(KeyDown(Backspace); modifiers=[Control], action_dispatch=TextToolMessage::DeletePreviousWord),
		entry!(KeyDown(Backspace); modifiers=[Command], action_dispatch=TextToolMessage::DeleteLine),
