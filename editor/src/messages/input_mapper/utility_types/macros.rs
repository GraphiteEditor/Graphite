...
macro_rules! mapping {
	[$($entry:expr_2021),* $(,)?] => {{
		...
		let mut triple_click = KeyMappingEntries::mouse_buttons_arrays();
		...
		for entry_slice in $entry {
			...
			for entry in entry_slice.into_iter() {
				...
				let corresponding_list = match entry.input {
					...
					InputMapperMessage::TripleClick(key) => &mut triple_click[key as usize],
		...
		(key_up, key_down, key_up_no_repeat, key_down_no_repeat, double_click, triple_click, wheel_scroll, pointer_move, pointer_shake)
	}};
}
