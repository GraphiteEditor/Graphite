...
use graphene_std::{Appearance, Artboard, Color, Cover, Coverage, Graphic};
use std::any::Any;
use std::sync::Arc;
...
fn generate_layout(introspected_data: &Arc<dyn std::any::Any + Send + Sync + 'static>, data: &mut LayoutData) -> Option<Vec<LayoutGroup>> {
	// `NodeIdPath` is interpreted as a path (e.g. the value produced by `path_of_subgraph`), shown as a
	// `List` where each item's NodeId resolves against the prefix made up of the items above it.
	if let Some(path) = introspected_data.downcast_ref::<NodeIdPath>() {
		return Some(table_node_id_path_layout_with_breadcrumb(&path.0, data));
	}
	// The path's bare list form, introspected off a leveled wire.
...
impl TableItemLayout for Graphic<'_> {
	...
	fn identifier(&self) -> String {
		// Our leaf variants hold the bare element, so they report the element's own identifier
		match self {
			Self::GraphicList(list) => list.identifier(),
			Self::Vector(vector) => vector.identifier(),
			Self::RasterCPU(raster) => raster.identifier(),
			Self::RasterGPU(raster) => raster.identifier(),
			Self::Color(color) => color.identifier(),
			Self::Gradient(gradient) => gradient.identifier(),
			Self::Text(text) => text.identifier(),
			Self::Stroke(stroke) => stroke.identifier(),
	...
	fn value_page(&self, data: &mut LayoutData) -> Vec<LayoutGroup> {
		match self {
			...
			Self::Vector(vector) => vector.layout_with_breadcrumb(data),
			Self::RasterCPU(raster) => raster.layout_with_breadcrumb(data),
			Self::RasterGPU(raster) => raster.layout_with_breadcrumb(data),
			Self::Color(color) => color.layout_with_breadcrumb(data),
			Self::Gradient(gradient) => gradient.layout_with_breadcrumb(data),
			Self::Text(text) => text.layout_with_breadcrumb(data),
			Self::Stroke(stroke) => stroke.layout_with_breadcrumb(data),
