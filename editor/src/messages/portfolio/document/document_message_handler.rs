...
impl DocumentMessageHandler {
	...
	/// to a shared `Solidify Stroke` node via two `Extract Element` nodes (indices 0 and 1).
	...
	fn handle_expand_fill_stroke_on_selected_layers(&mut self, responses: &mut VecDeque<Message>) {
		...
		let extract_element_definition = document_node_definitions::resolve_proto_node_type(graphene_std::graphic::extract_element::IDENTIFIER).expect("Extract Element node should exist");
		...
		for layer in selected_layers {
			...
			if has_fill && has_stroke {
				...
				let existing_index_template = extract_element_definition.node_template_input_override([None, Some(NodeInput::value(TaggedValue::F64(existing_index), false))]);
				...
				let new_index_template = extract_element_definition.node_template_input_override([None, Some(NodeInput::value(TaggedValue::F64(new_index), false))]);
