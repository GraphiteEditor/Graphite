	...
	fn process_message(&mut self, message: DocumentMessage, responses: &mut VecDeque<Message>, data: DocumentMessageData) {
		...
		match message {
			...
			DocumentMessage::SelectAllLayers => {
				...
				let all_visible_layers_except_artboards = metadata.all_layers().filter(move |&layer| !metadata.is_artboard(layer) && metadata.node_is_visible(layer.to_node()));
				let nodes = all_visible_layers_except_artboards.map(|layer| layer.to_node()).collect();
