...
pub(crate) fn diff_registries(stored: &document_graph::Registry, target: &document_graph::Registry) -> String {
...
fn diff_node(out: &mut String, stored: &document_graph::Node, target: &document_graph::Node) {
...
fn diff_network(out: &mut String, stored: &document_graph::Network, target: &document_graph::Network) {
...
/// ignoring LWW timestamps. Mirrors `document_graph`'s internal `resources_value_equal` for a single
/// entry, since that helper is crate-private and only operates over a whole store.
fn resource_value_equal(stored: &document_graph::ResourceEntry, target: &document_graph::ResourceEntry) -> bool {
...
fn diff_resource(out: &mut String, stored: &document_graph::ResourceEntry, target: &document_graph::ResourceEntry) {
...
fn diff_attributes(out: &mut String, label: &str, stored: &document_graph::Attributes, target: &document_graph::Attributes) {
