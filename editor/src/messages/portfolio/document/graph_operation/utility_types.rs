...
use graphene_std::vector::style::{GradientSpreadMethod, GradientType, GradientUnits, Stroke};
...
impl<'a> ModifyInputsContext<'a> {
	...
	pub fn fill_gradient_set(&mut self, gradient: GradientStops, gradient_type: GradientType, spread_method: GradientSpreadMethod, gradient_units: GradientUnits, transform: DAffine2) {
		let Some(fill_node_id) = self.existing_proto_node_id(graphene_std::vector_nodes::fill::IDENTIFIER, true) else {
			return;
		};
		...
		self.set_input_with_refresh(
			InputConnector::node(fill_node_id, graphene_std::vector::fill::GradientUnitsInput::INDEX),
