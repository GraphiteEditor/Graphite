...
pub fn update_transform(network_interface: &mut NodeNetworkInterface, node_id: &NodeId, transform: DAffine2) {
	...
	network_interface.set_input(InputConnector::node(*node_id, 1), NodeInput::value(TaggedValue::DVec2(translation), false), true);
	network_interface.set_input(InputConnector::node(*node_id, 2), NodeInput::value(TaggedValue::F64(angle), false), true);
	network_interface.set_input(InputConnector::node(*node_id, 3),  NodeInput::value(TaggedValue::DVec2(scale), false), true);
	network_interface.set_input(InputConnector::node(*node_id, 4), NodeInput::value(TaggedValue::DVec2(shear), false), true);
}
