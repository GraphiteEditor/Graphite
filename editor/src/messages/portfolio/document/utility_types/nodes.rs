...
impl SelectedNodes {
	pub fn layer_visible(&self, layer: LayerNodeIdentifier, network: &NodeNetwork, document_metadata: &DocumentMetadata) -> bool {
		!layer.ancestors(document_metadata).any(|layer| network.disabled.contains(&layer.to_node()))
	}

	pub fn selected_visible_layers<'a>(&'a self, network: &'a NodeNetwork, document_metadata: &'a DocumentMetadata) -> impl Iterator<Item = LayerNodeIdentifier> + '_ {
		self.selected_layers(document_metadata).filter(move |&layer| self.layer_visible(layer, network, document_metadata))
	}

	pub fn selected_layers<'a>(&'a self, document_metadata: &'a DocumentMetadata) -> impl Iterator<Item = LayerNodeIdentifier> + '_ {
		document_metadata.all_layers().filter(|layer| self.0.contains(&layer.to_node()))
	}

	pub fn selected_layers_except_artboards<'a>(&'a self, document_metadata: &'a DocumentMetadata) -> impl Iterator<Item = LayerNodeIdentifier> + '_ {
		self.selected_layers(document_metadata).filter(move |&layer| !document_metadata.is_artboard(layer))
	}

	pub fn selected_layers_contains(&self, layer: LayerNodeIdentifier, document_metadata: &DocumentMetadata) -> bool {
		self.selected_layers(document_metadata).any(|selected| selected == layer)
	}
