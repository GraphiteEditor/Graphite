...
use graphene_std::{
	node_graph_overlay::types::{FrontendGraphInputNew, FrontendGraphOutputNew, FrontendLayer, FrontendNode, FrontendNodeMetadata, FrontendNodeOrLayer, FrontendXY, NativeNodeToRender},
	uuid::NodeId,
};
use kurbo::BezPath;
...
impl NodeNetworkInterface {
	pub fn collect_nodes_new(&mut self, node_graph_errors: &GraphErrors, wire_style: GraphWireStyle, network_path: &[NodeId]) -> Vec<NativeNodeToRender> {
		...
		for (node_id, visible) in network.nodes.iter().map(|(node_id, node)| (*node_id, node.visible)).collect::<Vec<_>>() {
			...
			let node_or_layer = match self.is_layer(&node_id, network_path) {
				true => {
					...
					let layer = Some(FrontendLayerNew {
						bottom_input,
						side_input,
						output,
						position,
				...
				false => {
					...
					FrontendNodeOrLayer::Node(FrontendNodeNew {
						position,
						primary_input,
						primary_output,
						secondary_inputs,
			...
			let wires = (0..self.number_of_displayed_inputs(&node_id, network_path))
				.filter_map(|input_index| {
					self.wire_from_input_new(&InputConnector::node(node_id, input_index), wire_style, network_path)
						.filter(|_| {
			...
			let frontend_node_to_render = NativeNodeToRender { metadata, node_or_layer, wires };
