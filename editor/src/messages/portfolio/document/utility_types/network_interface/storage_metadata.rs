//! Bridge between `NodeNetworkInterface` and `document-graph`'s `NodeMetadataSource` trait.
...
/// Adapts a `&NodeNetworkInterface` to `document-graph`'s `NodeMetadataSource` (node/network metadata
...
pub fn network_ids_from_entries(network_entries: &[NetworkMetadataEntry]) -> HashMap<Vec<NodeId>, document_graph::NetworkId> {
...
/// `session.json` map keyed by the stable storage [`NetworkId`](document_graph::NetworkId), which
...
pub fn collect_network_view_settings(
	...
	network_ids: &HashMap<Vec<NodeId>, document_graph::NetworkId>,
) -> BTreeMap<document_graph::NetworkId, BTreeMap<String, serde_json::Value>> {
...
/// `network_path` to its [`NetworkId`](document_graph::NetworkId), and the matching inner map is decoded
...
pub fn apply_network_view_settings(
	...
	network_ids: &HashMap<Vec<NodeId>, document_graph::NetworkId>,
	network_view_settings: &BTreeMap<document_graph::NetworkId, BTreeMap<String, serde_json::Value>>,
) {
	for (network_path, network_id) in network_ids {
