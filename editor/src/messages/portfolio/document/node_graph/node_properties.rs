...
use graphene_std::vector::RepeatSpacingMethod;
...
pub(crate) fn repeat_properties(node_id: NodeId, context: &mut NodePropertiesContext) -> Vec<LayoutGroup> {
	...
	let mut widgets = vec![direction, LayoutGroup::Row { widgets: angle }, LayoutGroup::Row { widgets: count }, spacing_method];
	...
	if let Some(&TaggedValue::RepeatSpacingMethod(method)) = document_node.inputs.get(SPACING_METHOD_INDEX).and_then(|i| i.as_non_exposed_value()) {
		if matches!(method, RepeatSpacingMethod::Envelope | RepeatSpacingMethod::Gap) {
			let width = number_widget(ParameterWidgetsInfo::new(node_id, WIDTH_INDEX, true, context), NumberInput::default().min(0.).unit(" px"));
			widgets.push(LayoutGroup::Row { widgets: width });
		}
	}
...
pub(crate) fn circular_repeat_properties(node_id: NodeId, context: &mut NodePropertiesContext) -> Vec<LayoutGroup> {
	...
	let angular_spacing_method = enum_choice::<RepeatSpacingMethod>()
