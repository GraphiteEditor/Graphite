			...
			match concrete_type.alias.as_ref().map(|x| x.as_ref()) {
				...
				_ => {
					...
					match concrete_type.id {
						...
						// A leveled f64 arrives peeled to its element, so only a genuinely list-typed input reaches this arm
						Some(x) if id_is::<List<f64>>(x) => array_of_number_widget(default_info, TextInput::default()).into(),
...
pub fn stroke_properties(node_id: NodeId, context: &mut NodePropertiesContext) -> Vec<LayoutGroup> {
	...
	let paint_order = enum_choice::<PaintOrder>().for_socket(ParameterWidgetsInfo::new(node_id, PaintOrderInput, true, context)).property_row();
