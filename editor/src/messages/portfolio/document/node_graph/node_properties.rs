	...
	let widgets = match ty {
		Type::Concrete(concrete_type) => {
			match concrete_type.alias.as_ref().map(|x| x.as_ref()) {
				...
				Some("Resolution") => vec2_widget(default_info, "W", "H", " px", Some(64.)),
				...
				_ => {
					...
					match concrete_type.id {
						...
						Some(x) if x == TypeId::of::<DVec2>() => vec2_widget(default_info, "X", "Y", "", None),
						Some(x) if x == TypeId::of::<UVec2>() => vec2_widget(default_info, "X", "Y", "", Some(0.)),
						Some(x) if x == TypeId::of::<IVec2>() => vec2_widget(default_info, "X", "Y", "", None),
						Some(x) if x == TypeId::of::<Vec<f64>>() => vec_f64_input(default_info, TextInput::default()).into(),
						Some(x) if x == TypeId::of::<Vec<DVec2>>() => vec_dvec2_input(default_info, TextInput::default()).into(),
						Some(x) if x == TypeId::of::<Font>() => {
							let (font_widgets, style_widgets) = font_inputs(ParameterWidgetsInfo::new(document_node, node_id, index, name, description, false));
							font_widgets.into_iter().chain(style_widgets.unwrap_or_default()).collect::<Vec<_>>().into()
						}
						Some(x) if x == TypeId::of::<Curve>() => curves_widget(default_info),
						Some(x) if x == TypeId::of::<GradientStops>() => color_widget(default_info, ColorInput::default().allow_none(false)),
						Some(x) if x == TypeId::of::<VectorDataTable>() => vector_widget(default_info).into(),
						...
						Some(x) if x == TypeId::of::<Footprint>() => {
							let widgets = footprint_widget(default_info);
							let (last, rest) = widgets.split_last().expect("Footprint widget should return multiple rows");
							extra_widgets = rest.to_vec();
							last.clone()
						}
						Some(x) if x == TypeId::of::<BlendMode>() => blend_mode(default_info),
						Some(x) if x == TypeId::of::<RealTimeMode>() => real_time_mode(default_info),
						Some(x) if x == TypeId::of::<RedGreenBlue>() => color_channel(default_info),
						Some(x) if x == TypeId::of::<RedGreenBlueAlpha>() => rgba_channel(default_info),
						Some(x) if x == TypeId::of::<XY>() => xy_components(default_info),
						Some(x) if x == TypeId::of::<NoiseType>() => noise_type(default_info),
						Some(x) if x == TypeId::of::<FractalType>() => fractal_type(default_info, false),
						Some(x) if x == TypeId::of::<CellularDistanceFunction>() => cellular_distance_function(default_info, false),
						Some(x) if x == TypeId::of::<CellularReturnType>() => cellular_return_type(default_info, false),
						Some(x) if x == TypeId::of::<DomainWarpType>() => domain_warp_type(default_info, false),
						...
						.into(),
						Some(x) if x == TypeId::of::<BooleanOperation>() => boolean_operation_radio_buttons(default_info),
						Some(x) if x == TypeId::of::<CentroidType>() => centroid_widget(default_info),
						Some(x) if x == TypeId::of::<LuminanceCalculation>() => luminance_calculation(default_info),
...
pub fn footprint_widget(parameter_widgets_info: ParameterWidgetsInfo) -> Vec<LayoutGroup> {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn vec2_widget(parameter_widgets_info: ParameterWidgetsInfo, x: &str, y: &str, unit: &str, min: Option<f64>) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn vec_f64_input(parameter_widgets_info: ParameterWidgetsInfo, text_input: TextInput) -> Vec<WidgetHolder> {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn vec_dvec2_input(parameter_widgets_info: ParameterWidgetsInfo, text_props: TextInput) -> Vec<WidgetHolder> {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn color_channel(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn real_time_mode(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn rgba_channel(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn xy_components(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
// TODO: Generalize this instead of using a separate function per dropdown menu enum
pub fn noise_type(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
// TODO: Generalize this instead of using a separate function per dropdown menu enum
pub fn fractal_type(parameter_widgets_info: ParameterWidgetsInfo, disabled: bool) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
// TODO: Generalize this instead of using a separate function per dropdown menu enum
pub fn cellular_distance_function(parameter_widgets_info: ParameterWidgetsInfo, disabled: bool) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
// TODO: Generalize this instead of using a separate function per dropdown menu enum
pub fn cellular_return_type(parameter_widgets_info: ParameterWidgetsInfo, disabled: bool) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
// TODO: Generalize this instead of using a separate function per dropdown menu enum
pub fn domain_warp_type(parameter_widgets_info: ParameterWidgetsInfo, disabled: bool) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
// TODO: Generalize this instead of using a separate function per dropdown menu enum
pub fn blend_mode(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn luminance_calculation(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn boolean_operation_radio_buttons(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn curves_widget(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
...
pub fn centroid_widget(parameter_widgets_info: ParameterWidgetsInfo) -> LayoutGroup {
	let ParameterWidgetsInfo { document_node, node_id, index, .. } = parameter_widgets_info;
	...
	if let Some(&TaggedValue::GridType(grid_type)) = grid_type_input.as_non_exposed_value() {
		match grid_type {
			GridType::Rectangular => {
				let spacing = vec2_widget(ParameterWidgetsInfo::from_index(document_node, node_id, spacing_index, true, context), "W", "H", " px", Some(0.));
			...
			GridType::Isometric => {
				...
				let angles = vec2_widget(ParameterWidgetsInfo::from_index(document_node, node_id, angles_index, true, context), "", "", "°", None);
