	...
	fn process_message(&mut self, message: NodeGraphMessage, responses: &mut VecDeque<Message>, data: NodeGraphHandlerData<'a>) {
		...
		match message {
			...
			NodeGraphMessage::SetInputValue { node_id, input_index, value } => {
				...
				responses.add(PropertiesPanelMessage::Refresh);
				if (network_interface.get_reference(&node_id) != "Imaginate" || input_index == 0) && network_interface.connected_to_output(&node_id) {
					responses.add(NodeGraphMessage::RunDocumentGraph);
				}
			}
			NodeGraphMessage::SetInput {
				...
				network_interface.set_input(input_connector, input, skip_rerender, use_document_network);
			}
	...
	fn collect_nodes(&self, network_interface: &NodeNetworkInterface, wires: &[FrontendNodeWire]) -> Vec<FrontendNode> {
		...
		for (&node_id, node) in &network.nodes {
			...
			nodes.push(FrontendNode {
				...
				alias: network_interface.untitled_layer_label(&node_id),
				reference,
				primary_input,
				exposed_inputs,
				primary_output,
