...
impl NodeGraphMessageHandler {
	...
	fn send_graph(
		...
		document_metadata: &mut DocumentMetadata,
		selected_nodes: &mut SelectedNodes,
		collapsed: &CollapsedLayers,
		responses: &mut VecDeque<Message>,
	) {
		document_metadata.load_structure(network, selected_nodes);
		...
		for (&node_id, node) in &network.nodes {
			...
			if node.is_layer() {
				...
				let data = LayerPanelEntry {
					...
					expanded: layer.has_children(document_metadata) && !collapsed.0.contains(&layer),
