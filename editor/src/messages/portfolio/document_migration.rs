...
pub fn document_migration_upgrades(document: &mut DocumentMessageHandler, reset_node_definitions_on_open: bool, reset_layer_definitions: bool) {
	...
	// This must run as a pre-pass before the recursive iteration below: replacing the outer Transform's network impl orphans its child paths, and the recursive iteration would log errors for those stale paths.
	...
	// Pre-pass for the same reason as the Transform migration above: replacing the outer Image's network impl orphans its child paths.
...
mod tests {
	...
	fn every_legacy_text_shape_gains_its_geometry_converter() {
		...
		for shape in [4, 6, 8, 9, 10] {
			...
			document_migration_upgrades(&mut document, false, false);
