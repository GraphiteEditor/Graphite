...
const NODE_REPLACEMENTS: &[NodeReplacement<'static>] = &[
	...
	NodeReplacement {
		node: graphene_std::repeat::radial_repeat::IDENTIFIER,
	...
	NodeReplacement {
		node: graphene_std::vector::read_index::IDENTIFIER,
	...
	NodeReplacement {
		node: graphene_std::repeat::pointwise_repeat::IDENTIFIER,
		aliases: &[
			"graphene_core::vector::InstanceOnPointsNode",
			"core_types::vector::InstanceOnPointsNode",
			"graphene_core::vector::CopyToPointsNode",
	...
	NodeReplacement {
		node: graphene_std::vector::read_position::IDENTIFIER,
		aliases: &["graphene_core::vector::InstancePositionNode", "core_types::vector::InstancePositionNode"],
	},
	NodeReplacement {
		node: graphene_std::vector::read_vector::IDENTIFIER,
	...
	NodeReplacement {
		node: graphene_std::repeat::mirror_repeat::IDENTIFIER,
...
fn migrate_node(node_id: &NodeId, node: &DocumentNode, network_path: &[NodeId], document: &mut DocumentMessageHandler, reset_node_definitions_on_open: bool) -> Option<()> {
	...
	if reference == DefinitionIdentifier::ProtoNode(graphene_std::repeat::mirror_repeat::IDENTIFIER) && inputs_count == 3 {
		let mut node_template = resolve_document_node_type(&reference)?.default_node_template();
		document.network_interface.replace_implementation(node_id, network_path, &mut node_template);
	...
	if reference == DefinitionIdentifier::ProtoNode(graphene_std::repeat::mirror_repeat::IDENTIFIER) && inputs_count == 4 {
		let mut node_template = resolve_document_node_type(&reference)?.default_node_template();
		document.network_interface.replace_implementation(node_id, network_path, &mut node_template);
	...
	if reference == DefinitionIdentifier::ProtoNode(graphene_std::repeat::pointwise_repeat::IDENTIFIER) && inputs_count == 2 {
		let mut node_template = resolve_document_node_type(&reference)?.default_node_template();
		document.network_interface.replace_implementation(node_id, network_path, &mut node_template);
	...
	if reference == DefinitionIdentifier::ProtoNode(graphene_std::vector::read_index::IDENTIFIER) && inputs_count == 0 {
		let mut node_template = resolve_document_node_type(&reference)?.default_node_template();
		document.network_interface.replace_implementation(node_id, network_path, &mut node_template);
	...
	if reference == DefinitionIdentifier::ProtoNode(graphene_std::vector_nodes::instance::read_position::IDENTIFIER) && inputs_count < 2 {
		let mut node_template = resolve_document_node_type(&reference)?.default_node_template();
		document.network_interface.replace_implementation(node_id, network_path, &mut node_template);
		let _ = document.network_interface.replace_inputs(node_id, network_path, &mut node_template);
