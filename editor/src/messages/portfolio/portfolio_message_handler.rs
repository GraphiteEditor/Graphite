	...
	fn process_message(&mut self, message: PortfolioMessage, responses: &mut VecDeque<Message>, data: PortfolioMessageData) {
		...
		match message {
			...
			PortfolioMessage::OpenDocumentFileWithId {
				...
				let (_, upgrade_flags, _) = upgrade::detect_needed_upgrades(&document_name, &document_serialized_content);

				let document = upgrade::process_open_document_file_with_id(document_name.clone(), document_serialized_content);
				...
				// Check if vector manipulation format upgrade is needed
				if upgrade_flags.upgrade_vector_manipulation_format {
					let updated_document_serialized_content = document.serialize_document();
					if !updated_document_serialized_content.is_empty() {
						responses.add(FrontendMessage::TriggerUpgradeDocumentToVectorManipulationFormat {
							document_id,
							document_name,
							document_is_auto_saved,
							document_is_saved,
							document_serialized_content: updated_document_serialized_content,
						});
						return;
					}
				}
				// Set document state and load it
