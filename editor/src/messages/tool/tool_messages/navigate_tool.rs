...
impl Fsm for NavigateToolFsmState {
	...
	fn transition(
		...
		match navigate {
			NavigateToolMessage::PointerUp { zoom_in } => {
				if self == Self::ZoomOrClickZooming {
	...
	fn update_hints(&self, responses: &mut VecDeque<Message>) {
		let hint_data = match self {
			Self::Ready | Self::ZoomOrClickZooming => HintData(vec![
				HintGroup(vec![
	...
	fn update_cursor(&self, responses: &mut VecDeque<Message>) {
		let cursor = match *self {
			Self::Ready => MouseCursorIcon::ZoomIn,
			Self::Tilting => MouseCursorIcon::Default,
			Self::Zooming | Self::ZoomOrClickZooming => MouseCursorIcon::ZoomIn,
		};
