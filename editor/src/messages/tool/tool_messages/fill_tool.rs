...
mod test_fill {
	...
	async fn get_fills(editor: &mut EditorTestUtils) -> Vec<List<Graphic>> {
		let instrumented = match editor.eval_graph().await {
			Ok(instrumented) => instrumented,
			Err(e) => panic!("Failed to evaluate graph: {e}"),
		};

		instrumented.grab_all_input::<fill::FillInput<List<Graphic>>>(&editor.runtime).collect()
	}
	...
	async fn primary() {
		...
		let Some(Graphic::Color(colors)) = fills.first().unwrap().element(0) else {
			panic!("Expected a solid color fill")
		};
		let color = colors.element(0).expect("Color is stored in the list");
