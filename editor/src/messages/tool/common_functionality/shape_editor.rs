...
// Vec<LayerId> represents a layer path, ManipulatorGroupId represents the id of a manipulator group within the layer,
		...
		for (path, state) in &self.selected_shape_state {
			...
			for subpath in &vector_data.subpaths {
				for manipulator_group in subpath.manipulator_groups() {
					...
					let Some(new_opposing_handle_length) = opposing_handle_lengths.get(&manipulator_group.id) else { continue };
					...
					if let Some(new_opposing_handle_length) = new_opposing_handle_length {
						let Some(opposing_handle_position) = single_selected_handle.opposite().get_position(manipulator_group) else { continue };
						let Some(offset) = (opposing_handle_position - manipulator_group.anchor).try_normalize() else { continue };

						let position = manipulator_group.anchor + offset * (*new_opposing_handle_length);
