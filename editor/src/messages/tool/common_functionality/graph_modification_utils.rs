...
pub fn get_text(layer: LayerNodeIdentifier, network_interface: &NodeNetworkInterface) -> Option<(&String, &Font, f64, f64, f64, Option<f64>, Option<f64>)> {
	...
	let Some(&TaggedValue::OptionalF64(height)) = inputs[7].as_value() else { return None };

	Some((text, font, font_size, line_height_ratio, character_spacing, line_width, height))
}
