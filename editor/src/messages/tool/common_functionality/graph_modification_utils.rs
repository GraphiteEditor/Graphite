...
pub fn get_text(layer: LayerNodeIdentifier, document_network: &NodeNetwork) -> Option<(&graphene_core::text::RichText, f32, &VectorData)> {
	...
	let NodeInput::Value {
		tagged_value: TaggedValue::F32(line_width),
		..
	} = &inputs[2]
	else {
		return None;
