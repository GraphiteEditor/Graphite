...
impl Resize {
	...
	pub fn start(&mut self, document: &DocumentMessageHandler, mouse_position: DVec2) {
		let snap_targets = snapping::get_snap_targets(document, &[], &[]);

		let snapped_position = snapping::snap_position(&snap_targets, document, mouse_position);
	...
	pub fn calculate_transform(&self, center: Key, lock_ratio: Key, ipp: &InputPreprocessor, document: &DocumentMessageHandler) -> Option<Message> {
		if let (Some(path), Some(snap_targets)) = (&self.path, &self.snap_targets) {
			let mut start = self.drag_start;
			let stop = snapping::snap_position(snap_targets, document, ipp.mouse.position);
