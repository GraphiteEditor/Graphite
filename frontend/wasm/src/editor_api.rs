...
impl JsEditorHandle {
	...
	pub fn set_image_blob_url(&self, layer_path: Vec<LayerId>, blob_url: String, width: f64, height: f64) {
		let resolution = (width, height);
		let message = DocumentMessage::SetImageBlobUrl { layer_path, blob_url, resolution };
		self.dispatch(message);
	}

	/// Sends the blob URL generated by JS to the AI Artist layer in the respective document
	#[wasm_bindgen(js_name = setAIArtistImageData)]
	pub fn set_ai_artist_image_data(&self, document_id: u64, layer_path: Vec<LayerId>, image_data: Vec<u8>) {
		let message = PortfolioMessage::SetAiArtistImageData { document_id, layer_path, image_data };
		self.dispatch(message);
	}

	/// Sends the blob URL generated by JS to the AI Artist layer in the respective document
	#[wasm_bindgen(js_name = setAIArtistBlobURL)]
	pub fn set_ai_artist_blob_url(&self, document_id: u64, layer_path: Vec<LayerId>, blob_url: String, width: f64, height: f64) {
		let resolution = (width, height);
		let message = PortfolioMessage::SetAiArtistBlobUrl {
			document_id,
			layer_path,
			blob_url,
			resolution,
	...
	pub fn set_ai_artist_generating_status(&self, document_id: u64, path: Vec<LayerId>, percent: Option<f64>, generating: bool) {
		let message = PortfolioMessage::SetAiArtistGeneratingStatus {
			document_id,
			path,
			percent,
			generating,
