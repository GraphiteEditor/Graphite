...
export function createInputManager(editor: Editor, dialog: DialogState, portfolio: PortfolioState, document: DocumentState, fullscreen: FullscreenState): () => void {
	...
	function onPointerDown(e: PointerEvent) {
		...
		const { target } = e;
		const isTargetingCanvas = target instanceof Element && target.closest("[data-viewport], [data-node-graph], [data-viewport-container]");
	...
	function potentiallyRestoreCanvasFocus(e: Event) {
		const { target } = e;
		const newInCanvasArea = (target instanceof Element && target.closest("[data-viewport], [data-graph], [data-viewport-container]")) instanceof Element && !targetIsTextField(window.document.activeElement || undefined);
