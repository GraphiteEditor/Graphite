...
type JsMessageCallbackMap = Record<string, JsMessageCallback | undefined>;

// eslint-disable-next-line @typescript-eslint/explicit-function-return-type
export function createSubscriptionRouter() {
	const subscriptions: JsMessageCallbackMap = {};

	const subscribeJsMessage = <Args extends unknown[]>(messageType: new (...args: Args) => FrontendMessage, callback: JsMessageCallback): void => {
		subscriptions[messageType.name] = callback;
	};

	const handleJsMessage = (messageType: JsMessageType, messageData: any): void => {
		...
		//const message = messageIsClass ? plainToInstance(messageMaker, messageData) : messageMaker(messageData, wasm, instance);
