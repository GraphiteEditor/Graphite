...
async openAutoSavedDocument(document_id: string, document_name: string, document_is_saved: Promise<boolean>, document_serialized_content: string) {
  dispatch_response(await commands.openAutoSavedDocument( document_id,  document_name,  await document_is_saved,  document_serialized_content ));
}
/**
* @param {string} document_id
*/
...
*/
  async selectLayer(layer_path: string[], ctrl: Promise<boolean>, shift: boolean) {
	  dispatch_response(await commands.selectLayer(layer_path, await ctrl, shift));
...
* @param {number[]} image_data
* @param {number} width
* @param {number} height
*/
  async setImaginateImageData(document_id: string, layer_path: string[], node_path: string[], image_data: number[], width: number, height: number) {
...
* @param {string} status
*/
  async setImaginateGeneratingStatus(document_id: string, layer_path: string[], node_path: string[], percent: number | undefined, status: string) {
...
*/
  async processNodeGraphFrame(document_id: string, layer_path: string[], image_data: number[], width: number, height: number, imaginate_node?: string[]) {
...
*/
  async pasteImage(image_data: number[], width: number, height: number, mouse_x?: number, mouse_y?: number) {
